...
# adapted from official panopticapi by removing intersection from IOU computation
# ==
...
def pq_compute_single_core(proc_id, annotation_set, gt_folder, pred_folder, categories):
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        pan_gt = np.array(
            Image.open(os.path.join(gt_folder, gt_ann["file_name"])), dtype=np.uint32
        )
        ...
        if len(pred_labels_set) != 0:
            raise KeyError(
                "In the image with ID {} the following segment IDs {} are presented in JSON and not "
        ...
        for label_tuple, intersection in gt_pred_map.items():
            ...
            if iou > 1.0:
                _logger.warning(
                    f'gt file: {gt_ann["file_name"]} and pred file: {pred_ann["file_name"]} \n IOU score '
                    f"of {iou} with the pred label is {pred_label} with score "
                    f'{pred_segms[pred_label]["area"]}, the gt_label is {pred_label} with '
                    f'score {gt_segms[gt_label]["area"]} and the scores for the intersection '
                    f"is {intersection}, the unkonw score is {gt_pred_map.get((VOID, pred_label), 0)},"
                    f" pan_gt shape {pan_gt.shape} and pan_pred shape is {pan_pred.shape} "
                )
            if iou > 0.5:
