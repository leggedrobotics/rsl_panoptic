...
flags.DEFINE_bool(
    "use_two_frames",
    False,
    "Flag to separate between 1 frame "
...
def _get_images_per_shard(
    step_root: str, dataset_split: str, sharded_by_sequence: bool
    ...
    for i, name in enumerate(filenames):
        ...
        shard_data = shard_data or (
            sharded_by_sequence and sequence_ids[i + 1] != sequence_ids[i]
        )
        # Sharded evenly.
        shard_data = shard_data or (
            not sharded_by_sequence and len(images_per_shard) == num_per_even_shard
        )
        if shard_data:
...
def _convert_dataset(
    step_root: str, dataset_split: str, output_dir: str, use_two_frames: bool = False
):
    ...
    create_tfrecord_per_sequence = ("train" not in dataset_split) and use_two_frames
    is_testing = "test" in dataset_split

    image_files_per_shard = list(
        _get_images_per_shard(
            step_root, dataset_split, sharded_by_sequence=create_tfrecord_per_sequence
        )
    )
        ...
        with tf.io.TFRecordWriter(output_filename) as tfrecord_writer:
            for image_path in image_list:
                ...
                example = _create_panoptic_tfexample(
                    image_path, panoptic_map_path, use_two_frames, is_testing
                )
