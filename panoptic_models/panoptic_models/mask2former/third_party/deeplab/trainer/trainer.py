...
class WarmUp(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(
        self, initial_learning_rate, decay_schedule_fn, warmup_steps, name=None
    ):
...
def _create_optimizer(
    solver_config: config_pb2.SolverOptions, learning_rate_multiplier: float = 1.0
    ...
    if solver_config.learning_policy == "poly":
        ...
        raise ValueError(
            "Learning rate policy %s is not supported." % solver_config.learning_policy
        )
    ...
    if solver_config.optimizer == "adam":
        ...
        return tf.keras.optimizers.SGD(learning_rate=lr_scheduler, momentum=0.9)
...
class Trainer(orbit.StandardTrainer):
    ...
    def __init__(
        ...
        support_panoptic = (
            common.TASK_PANOPTIC_SEGMENTATION in utils.get_supported_tasks(config)
        )
    ...
    def _apply_gradients_to_optimizers(self, gradients_and_variables):
        ...
        if self._backbone_optimizer is None:
            ...
            for gradient, variable in gradients_and_variables:
                if runner_utils.check_if_variable_in_backbone(
    ...
    def _train_step(self, inputs: Dict[Text, Any]):
        ...
        with tf.GradientTape() as tape:
            ...
            for name, loss in loss_dict.items():
                ...
                average_loss_dict[name] = tf.where(
                    tf.math.is_nan(averaged_loss), 0.0, averaged_loss
                )
