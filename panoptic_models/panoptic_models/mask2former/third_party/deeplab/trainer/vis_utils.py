...
def create_rgb_from_instance_map(instance_map):
    ...
    if np.max(instance_map) < 512:
        ...
        instance_image = np.zeros(
            [instanc_map_shape[0], instanc_map_shape[1], 3], np.uint8
        )
...
def overlay_heatmap_on_image(heatmap, input_image, dpi=80.0, add_color_bar=False):
...
def label_to_color_image(label, colormap_name="cityscapes"):
    ...
    if np.max(label) >= 256:
        raise ValueError("label value too large: {} >= 256.".format(np.max(label)))
    ...
    if colormap_name == "cityscapes":
        ...
        raise ValueError("Could not find a colormap for dataset %s." % colormap_name)
...
def save_parsing_result(
    ...
    if parsing_result.ndim != 2:
        raise ValueError(
            "Expect 2-D parsing result. Got {}".format(parsing_result.shape)
        )
    ...
    if np.max(semantic_result) >= colormap_max_value:
        raise ValueError(
            "Predicted semantic value too large: {} >= {}.".format(
...
def perturb_color(color, noise, used_colors=None, max_trials=50, random_state=None):
