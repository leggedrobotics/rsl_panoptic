...
class InstanceAveragePrecision(tf.keras.metrics.Metric):
    ...
    def update_state(
        self,
        groundtruth_boxes: tf.Tensor,
        groundtruth_classes: tf.Tensor,
        groundtruth_masks: tf.Tensor,
        groundtruth_is_crowd: tf.Tensor,
        detection_masks: tf.Tensor,
...
def _instance_masks(
    panoptic_label_map: tf.Tensor, instance_panoptic_labels: tf.Tensor
) -> tf.Tensor:
    ...
    return tf.math.equal(
        ...
        tf.reshape(instance_panoptic_labels, [tf.size(instance_panoptic_labels), 1, 1]),
    )
...
class PanopticInstanceAveragePrecision(tf.keras.metrics.Metric):
    ...
    def update_state(
        ...
        is_crowd_map: Optional[tf.Tensor] = None,
    ) -> None:
        ...
        Args:
          ...
          is_crowd_map: A 2D boolean tensor. Where it is True, the instance in that
            ...
            crowds.
        """
        classes_to_ignore = tf.convert_to_tensor(
            [self.ignored_label] + list(self.stuff_list), tf.int32
        )
        (
            gt_unique_labels,
            gt_box_coords,
        ) = panoptic_instances.instance_boxes_from_masks(
        ...
        (pred_unique_labels, pred_scores) = panoptic_instances.combined_instance_scores(
            predicted_panoptic,
        ...
        pred_class_is_ignored = tf.math.reduce_any(
            tf.math.equal(
                tf.expand_dims(pred_classes, 1), tf.expand_dims(classes_to_ignore, 0)
            ),
            axis=1,
        )
        ...
        self.detection_metric.update_state(
            gt_box_coords,
            gt_classes,
            gt_masks,
            gt_is_crowd,
