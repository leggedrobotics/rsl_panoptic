...
def instances_without_ignore_categories(
    panoptic_labels: tf.Tensor,
    ignore_categories: Union[tf.Tensor, Iterable[int]],
    ...
    instance_is_ignored = tf.math.reduce_any(
        tf.equal(
            tf.expand_dims(instance_category, 1), tf.expand_dims(ignore_categories, 0)
        ),
        axis=1,
    )
...
def instance_boxes_from_masks(
    ...
    instance_index_repeated = _broadcast_over_instances(instance_index, num_instances)
...
def per_instance_masks(
    panoptic_labels: tf.Tensor,
    instance_panoptic_labels: tf.Tensor,
    out_dtype: tf.dtypes.DType = tf.bool,
) -> tf.Tensor:
...
def _average_per_instance(
    map_tensor: tf.Tensor,
    panoptic_labels: tf.Tensor,
    instance_panoptic_labels: tf.Tensor,
    ...
    num_instances = tf.size(instance_panoptic_labels)
    map_or_zero = tf.where(
        pixel_in_instance,
        tf.expand_dims(map_tensor, 0),
    ...
    instance_avg_prob = tf.divide(
        instance_total_prob, tf.cast(instance_area, map_dtype)
    )
...
def per_instance_semantic_probabilities(
    ...
    ignore_label: Union[tf.Tensor, int],
) -> tf.Tensor:
    ...
    pixel_semantic_probability = tf.where(
        semantic_label_map == ignore_label, 0.0, pixel_semantic_probability
    )

    instance_avg_prob = _average_per_instance(
...
def combined_instance_scores(
    ...
    instance_semantic_labels = tf.math.floordiv(
        instance_panoptic_labels, panoptic_divisor
    )
    instance_panoptic_labels = tf.boolean_mask(
        instance_panoptic_labels, instance_semantic_labels != ignore_label
    )
    instance_area = tf.boolean_mask(
        instance_area, instance_semantic_labels != ignore_label
    )
    ...
    instance_scores = _average_per_instance(
        instance_score_map, panoptic_labels, instance_panoptic_labels, instance_area
    )
...
def per_instance_is_crowd(
    is_crowd_map: tf.Tensor, id_map: tf.Tensor, output_ids: tf.Tensor
) -> tf.Tensor:
