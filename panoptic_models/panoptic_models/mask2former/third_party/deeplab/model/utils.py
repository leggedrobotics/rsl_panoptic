...
_INPUT_WITH_NEAREST_UPSAMPLING = (common.GT_INSTANCE_CENTER_KEY,)
...
def _scale_helper(value, scale):
    if isinstance(value, tf.Tensor):
        return tf.cast(
            (tf.cast(value, dtype=tf.float32) - 1.0) * scale + 1.0, dtype=tf.int32
        )
    else:
        return int((float(value) - 1.0) * scale + 1.0)


def scale_mutable_sequence(
    input_sequence: MutableSequence[int], scale: float
) -> MutableSequence[int]:
...
def undo_image_preprocessing(
    ...
    """
    if perform_crop:
        image_out = image_in[:, : regions_to_crop[0], : regions_to_crop[1], :]
    else:
        image_out = image_in
...
def undo_preprocessing(
    input_or_prediction_dict: MutableMapping[str, Any],
    regions_to_crop: List[int],
    output_shape: List[int],
    ...
    for key in input_or_prediction_dict.keys():
        if (
            key in _PREDICTION_WITH_NEAREST_UPSAMPLING
            or key in _INPUT_WITH_NEAREST_UPSAMPLING
        ):
            ...
            key in _PREDICTION_WITH_BILINEAR_UPSAMPLING
            or key in _INPUT_WITH_BILINEAR_UPSAMPLING
        ):
...
def add_zero_padding(input_tensor: tf.Tensor, kernel_size: int, rank: int) -> tf.Tensor:
    ...
    if rank == 3:
        return tf.pad(
            input_tensor, paddings=[[pad_begin, pad_end], [pad_begin, pad_end], [0, 0]]
        )
    else:
        return tf.pad(
            input_tensor,
...
def resize_and_rescale_offsets(input_tensor: tf.Tensor, target_size):
    ...
    target_y, target_x = tf.split(value=input_tensor, num_or_size_splits=2, axis=3)
    target_y *= scale_y
    target_x *= scale_x
...
def resize_align_corners(input_tensor, target_size, method="bilinear"):
    ...
    if method == "bilinear":
        ...
        raise ValueError(
            "The given method %s is not supported. Please use bilinear"
            " or nearest." % method
        )

    tf.debugging.assert_rank(
        input_tensor, 4, message="Input tensor to resize method should have rank of 4."
    )
...
def resize_bilinear(images, size, align_corners=True, name=None):
...
def make_divisible(
    value: float, divisor: int, min_value: Optional[float] = None
) -> int:
...
def get_low_level_conv_fusion_conv_current_names(index):
    return ("_low_level_conv{}".format(index + 1), "_fusion_conv{}".format(index + 1))
...
def get_conv_bn_act_current_name(index, use_bn, activation):
    ...
    if (
        ...
        and activation.lower() != "none"
        and activation.lower() != "linear"
    ):
        name += "_act"
    return name
...
def strided_downsample(input_tensor, target_size):
    ...
    if (input_height - 1) % (target_height - 1) or (input_width - 1) % (
        target_width - 1
    ):
        raise ValueError(
            "The input cannot be downsampled with integer striding. "
    ...
    if stride_height != stride_width:
        raise ValueError(
            "The height axis stride does not equal to the width axis " "stride."
        )
...
def get_stuff_class_ids(
    num_thing_stuff_classes: int, thing_class_ids: List[int], void_label: int
) -> List[int]:
    ...
    if void_label >= num_thing_stuff_classes:
        ...
        thing_stuff_class_ids = [
            _ for _ in range(num_thing_stuff_classes + 1) if _ is not void_label
        ]
    return sorted(set(thing_stuff_class_ids) - set(thing_class_ids))


def get_supported_tasks(config: config_pb2.ExperimentOptions) -> Set[str]:
    ...
    is_panoptic = (
        (config.model_options.panoptic_deeplab.instance.enable and is_panoptic_deeplab)
        or is_motion_deeplab
        or is_max_deeplab
        or is_vip_deeplab
    )
    if is_panoptic:
