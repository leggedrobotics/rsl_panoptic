...
def _apply_func_with_prob(func, image, args, prob, label):
    ...
    augmented_image, augmented_label = tf.cond(
        should_apply_op, lambda: func(image, label, *args), lambda: (image, label)
    )
...
def build_and_apply_autoaugment_policy(policies, image, label, ignore_label):
    ...
    for policy in policies:
        ...
        for policy_info in policy:
            policy_info = list(policy_info) + [replace_value, ignore_label]
...
def distort_image_with_autoaugment(image, label, ignore_label, augmentation_name=None):
    ...
    if augmentation_name:
        ...
        return build_and_apply_autoaugment_policy(policy, image, label, ignore_label)
    return image, label
