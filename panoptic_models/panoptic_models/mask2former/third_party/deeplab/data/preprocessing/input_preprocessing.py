...
def _pad_image_and_label(
    ...
    assert_right_padding = tf.assert_greater(
        right_padding,
        -1,
        "The padding configuration is not valid. Please either"
    ...
    with tf.control_dependencies([assert_bottom_padding, assert_right_padding]):
        paddings = [
            [offset_height, bottom_padding],
            [offset_width, right_padding],
            [0, 0],
        ]
...
def preprocess_image_and_label(
    ...
    if depth is not None:
        if any(value != 0 for value in min_resize_value) or any(
            value != 0 for value in max_resize_value
        ):
            raise ValueError(
    ...
    if not is_training:
        ...
        processed_image, label = _pad_image_and_label(
            processed_image,
            label,
            offset_height,
            offset_width,
    ...
    if autoaugment_policy_name:
        ...
        if processed_prev_image is not None:
            processed_prev_image, prev_label = _autoaugment_helper(
                processed_prev_image, prev_label, ignore_label, autoaugment_policy_name
            )
