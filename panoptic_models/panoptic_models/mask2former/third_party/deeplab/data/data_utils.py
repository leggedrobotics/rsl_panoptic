...
def create_features(
    image_data, image_format, filename, label_data=None, label_format=None
):
    ...
    if label_format == "png":
        ...
        if (label_height, label_width) != (height, width):
            raise ValueError(
                "Image (%s) and label (%s) shape mismatch"
        ...
        if len(label_data) != expected_label_size:
            raise ValueError(
                "Expects raw label data length %d, gets %d"
...
def create_tfexample(
    ...
    feature_dict = create_features(
        image_data, image_format, filename, label_data, label_format
    )
    return tf.train.Example(features=tf.train.Features(feature=feature_dict))


def create_video_tfexample(
    image_data,
    image_format,
    filename,
    sequence_id,
...
def create_video_and_depth_tfexample(
    image_data,
    image_format,
    filename,
    sequence_id,
    ...
    if depth_data is not None:
        feature_dict[common.KEY_ENCODED_DEPTH] = _bytes_list_feature(depth_data)
        feature_dict[common.KEY_DEPTH_FORMAT] = _bytes_list_feature(depth_format)
    return tf.train.Example(features=tf.train.Features(feature=feature_dict))
...
class SegmentationDecoder(object):
    ...
    def __call__(self, serialized_example):
        ...
        return_dict = {
            ...
            "height": tf.cast(parsed_tensors[common.KEY_IMAGE_HEIGHT], dtype=tf.int32),
        ...
        if self._decode_groundtruth_label:
            return_dict["label"] = self._decode_label(
                parsed_tensors, common.KEY_ENCODED_LABEL
            )
        if self._is_video_dataset:
