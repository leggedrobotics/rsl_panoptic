...
class AddAbsolutePositionalEncoding(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if not any(
            [
                positional_encoding_type is None,
        ...
        self._kernel_regularizer = tf.keras.regularizers.l2(conv_kernel_weight_decay)
    ...
    def call(self, features, training=False):
        ...
        """
        if (
            self._positional_encoding_type is None
            or self._positional_encoding_type.lower() == "none"
        ):
            return features
        elif self._positional_encoding_type.lower() == "2d":
            positional_encoding = self._batch_norm(self._embeddings, training=training)
        elif self._positional_encoding_type.lower() == "1d":
            ...
            positional_encoding = (
                height_axis_positional_encoding + width_axis_positional_encoding
            )
