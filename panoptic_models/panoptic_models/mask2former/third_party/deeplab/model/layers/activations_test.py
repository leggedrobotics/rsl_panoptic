...
class ActivationsTest(tf.test.TestCase):
    def test_gelu(self):
        expected_data = [
            [0.14967535, 0.0, -0.10032465],
            [-0.15880796, -0.04540223, 2.9963627],
        ]
        gelu_data = activations.gelu([[0.25, 0, -0.25], [-1, -2, 3]], approximate=True)
        self.assertAllClose(expected_data, gelu_data)
        gelu_data_via_get_activation = activations.get_activation("approximated_gelu")(
            [[0.25, 0, -0.25], [-1, -2, 3]]
        )
