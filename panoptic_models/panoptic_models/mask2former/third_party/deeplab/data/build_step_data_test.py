...
class BuildStepDataTest(tf.test.TestCase):
    ...
    def _create_images(self, split):
        image_path = os.path.join(
            self.data_dir, build_step_data._IMAGE_FOLDER_NAME, split, self.sequence_id
        )
    ...
    def _create_image_and_panoptic_map(self, image_path, panoptic_path, image_id):
        ...
        with tf.io.gfile.GFile(
            os.path.join(image_path, "%06d.png" % image_id), "wb"
        ) as f:
            Image.fromarray(image).save(f, format="PNG")

        # Dummy panoptic map.
        semantic = np.random.randint(0, 20, (self.height, self.width), dtype=np.int32)
        instance = np.random.randint(0, 1000, (self.height, self.width), dtype=np.int32)
        encoded_panoptic_map = np.dstack(
            (semantic, instance // 256, instance % 256)
        ).astype(np.uint8)
        with tf.io.gfile.GFile(
            os.path.join(panoptic_path, "%06d.png" % image_id), "wb"
        ) as f:
    ...
    def test_build_step_dataset_correct(self):
        ...
        build_step_data._convert_dataset(
            step_root=self.data_dir, dataset_split=split, output_dir=FLAGS.test_tmpdir
        )
        # We will have 2 shards with each shard containing 1 image.
        num_shards = 2
        output_record = os.path.join(
            FLAGS.test_tmpdir,
            build_step_data._TF_RECORD_PATTERN % (split, 0, num_shards),
        )
