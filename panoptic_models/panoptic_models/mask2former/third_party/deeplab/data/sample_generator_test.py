...
flags.DEFINE_bool(
    "update_golden_data", False, "Whether or not to update the golden data for testing."
)
...
class PanopticSampleGeneratorTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._test_img_data_dir = os.path.join(
            FLAGS.test_srcdir, FLAGS.panoptic_annotation_data, _IMAGE_FOLDER
        )
        self._test_gt_data_dir = os.path.join(
            FLAGS.test_srcdir, FLAGS.panoptic_annotation_data
        )
        self._test_target_data_dir = os.path.join(
            FLAGS.test_srcdir, FLAGS.panoptic_annotation_data, _TARGET_FOLDER
        )
        ...
        self._label = tf.expand_dims(
            tf.convert_to_tensor(np.dot(np.array(label), [1, 256, 256 * 256])), -1
        )
    ...
    def test_input_generator(self):
        ...
        self.assertListEqual(sample[common.GT_SEMANTIC_KEY].shape.as_list(), [769, 769])
        self.assertListEqual(sample[common.GT_PANOPTIC_KEY].shape.as_list(), [769, 769])
        self.assertListEqual(
            sample[common.GT_INSTANCE_CENTER_KEY].shape.as_list(), [769, 769]
        )
        self.assertListEqual(
            sample[common.GT_INSTANCE_REGRESSION_KEY].shape.as_list(), [769, 769, 2]
        )
        self.assertListEqual(sample[common.GT_IS_CROWD].shape.as_list(), [769, 769])
        self.assertListEqual(
            sample[common.GT_THING_ID_MASK_KEY].shape.as_list(), [769, 769]
        )
        self.assertListEqual(
            sample[common.GT_THING_ID_CLASS_KEY].shape.as_list(), [128]
        )
        self.assertListEqual(
            sample[common.SEMANTIC_LOSS_WEIGHT_KEY].shape.as_list(), [769, 769]
        )
        self.assertListEqual(
            sample[common.CENTER_LOSS_WEIGHT_KEY].shape.as_list(), [769, 769]
        )
        self.assertListEqual(
            sample[common.REGRESSION_LOSS_WEIGHT_KEY].shape.as_list(), [769, 769]
        )
        ...
        center_weights = tf.cast(sample[common.CENTER_LOSS_WEIGHT_KEY] * 255, tf.uint8)
        offset_weights = tf.cast(
        ...
        np.testing.assert_almost_equal(
            ...
            _get_groundtruth_image(
                image, self._test_target_data_dir + "rgb_target.png"
            ),
        )
        np.testing.assert_almost_equal(
            gt_sem.numpy(),
            _get_groundtruth_image(
                gt_sem, self._test_target_data_dir + "semantic_target.png"
            ),
        )
        ...
        np.testing.assert_almost_equal(
            ...
            _get_groundtruth_array(
                gt_pan, self._test_target_data_dir + "panoptic_target.npy"
            ),
        )
        np.testing.assert_almost_equal(
        ...
        np.testing.assert_almost_equal(
            gt_center.numpy(),
            _get_groundtruth_image(
                gt_center, self._test_target_data_dir + "center_target.png"
            ),
        )
        np.testing.assert_almost_equal(
        ...
        np.testing.assert_array_equal(
            gt_is_crowd.numpy(),
            _get_groundtruth_array(
                gt_is_crowd.numpy(), self._test_target_data_dir + "is_crowd.npy"
            ),
        )
        np.testing.assert_almost_equal(
            semantic_weights.numpy(),
            _get_groundtruth_image(
                semantic_weights, self._test_target_data_dir + "semantic_weights.png"
            ),
        )
        np.testing.assert_almost_equal(
            center_weights.numpy(),
            _get_groundtruth_image(
                center_weights, self._test_target_data_dir + "center_weights.png"
            ),
        )
        np.testing.assert_almost_equal(
            offset_weights.numpy(),
            _get_groundtruth_image(
                offset_weights, self._test_target_data_dir + "offset_weights.png"
            ),
        )
    ...
    def test_input_generator_eval(self):
        ...
        np.testing.assert_almost_equal(
            gt_sem_raw.numpy(),
            _get_groundtruth_image(
                gt_sem_raw, self._test_target_data_dir + "eval_semantic_target.png"
            ),
        )
        np.testing.assert_almost_equal(
            gt_pan_raw.numpy(),
            _get_groundtruth_array(
                gt_pan_raw, self._test_target_data_dir + "eval_panoptic_target.npy"
            ),
        )
        np.testing.assert_almost_equal(
            gt_is_crowd_raw.numpy(),
            _get_groundtruth_array(
                gt_is_crowd_raw, self._test_target_data_dir + "eval_is_crowd.npy"
            ),
        )
