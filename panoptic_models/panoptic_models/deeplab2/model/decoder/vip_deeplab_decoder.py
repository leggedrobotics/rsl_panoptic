...
from panoptic_models.deeplab2.model.decoder import panoptic_deeplab
...
class ViPDeepLabDecoder(layers.Layer):
    ...
    def __init__(
        ...
        if vip_deeplab_options.HasField("depth_head"):
            ...
            self._max_depth = vip_deeplab_options.depth_head.max_value_after_activation
            self._min_depth = vip_deeplab_options.depth_head.min_value_after_activation
        ...
        if vip_deeplab_options.instance.enable:
            ...
            if vip_deeplab_options.instance.HasField("instance_decoder_override"):
                decoder_options = vip_deeplab_options.instance.instance_decoder_override
            ...
            self._instance_regression_head = panoptic_deeplab.PanopticDeepLabSingleHead(
                ...
                conv_type=(vip_deeplab_options.instance.regression_head.head_conv_type),
                bn_layer=bn_layer,
            )
            ...
            if vip_deeplab_options.instance.HasField("next_regression_head"):
                ...
                self._next_instance_regression_head = panoptic_deeplab.PanopticDeepLabSingleHead(
                    (vip_deeplab_options.instance.next_regression_head.head_channels),
                    (vip_deeplab_options.instance.next_regression_head.output_channels),
                    common.PRED_NEXT_OFFSET_MAP_KEY,
                    name="next_instance_regression_head",
                    conv_type=(
                        vip_deeplab_options.instance.next_regression_head.head_conv_type
                    ),
                    bn_layer=bn_layer,
                )
    ...
    def checkpoint_items(self):
        items = {
            common.CKPT_SEMANTIC_DECODER: self._semantic_decoder,
            common.CKPT_SEMANTIC_HEAD_WITHOUT_LAST_LAYER: self._semantic_head.conv_block,
            common.CKPT_SEMANTIC_LAST_LAYER: self._semantic_head.final_conv,
        }
        if self._instance_decoder is not None:
            instance_items = {
                common.CKPT_INSTANCE_DECODER: self._instance_decoder,
                common.CKPT_INSTANCE_CENTER_HEAD_WITHOUT_LAST_LAYER: self._instance_center_head.conv_block,
                common.CKPT_INSTANCE_CENTER_HEAD_LAST_LAYER: self._instance_center_head.final_conv,
                common.CKPT_INSTANCE_REGRESSION_HEAD_WITHOUT_LAST_LAYER: self._instance_regression_head.conv_block,
                common.CKPT_INSTANCE_REGRESSION_HEAD_LAST_LAYER: self._instance_regression_head.final_conv,
            }
            items.update(instance_items)
        if self._next_instance_decoder is not None:
            next_instance_items = {
                common.CKPT_NEXT_INSTANCE_DECODER: self._next_instance_decoder,
                common.CKPT_NEXT_INSTANCE_REGRESSION_HEAD_WITHOUT_LAST_LAYER: self._next_instance_regression_head.conv_block,
                common.CKPT_NEXT_INSTANCE_REGRESSION_HEAD_LAST_LAYER: self._next_instance_regression_head.final_conv,
            }
            items.update(next_instance_items)
        if self._depth_head is not None:
            depth_items = {
                common.CKPT_DEPTH_HEAD_WITHOUT_LAST_LAYER: self._depth_head.conv_block,
                common.CKPT_DEPTH_HEAD_LAST_LAYER: self._depth_head.final_conv,
            }
    ...
    def call(self, features, next_features, training=False):
        ...
        if self._instance_decoder is not None:
            ...
            if results.keys() & instance_center_predictions.keys():
                raise ValueError(
                    "The keys of the semantic branch and the instance "
        ...
        if self._next_instance_decoder is not None:
            ...
            if results.keys() & next_regression_predictions.keys():
                raise ValueError(
                    "The keys of the next regresion branch overlap."
