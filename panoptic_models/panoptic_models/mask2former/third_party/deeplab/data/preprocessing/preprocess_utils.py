...
def random_crop(image_list, crop_height, crop_width):
    ...
    for i in range(len(image_list)):
        ...
        rank_assert = tf.Assert(
            ...
            [
                "Wrong rank for tensor %d in image_list [expected] [actual]",
                i,
                3,
                image_rank,
            ],
    ...
    for i in range(1, len(image_list)):
        ...
        height_assert = tf.Assert(
            ...
            [
                "Wrong height for tensor %d in image_list [expected][actual]",
                i,
                height,
                image_height,
            ],
...
def randomly_scale_image_and_label(image, label=None, scale=1.0):
    ...
    image = tf.squeeze(
        tf.compat.v1.image.resize_bilinear(
            tf.expand_dims(image, 0), new_dim, align_corners=True
        ),
        [0],
    )
    if label is not None:
        label = tf.compat.v1.image.resize(
            label, new_dim, method=get_label_resize_method(label), align_corners=True
        )
...
def _resize_to_match_min_size(input_shape, min_size):
    ...
    return (
        _scale_dim(input_height, scale_factor),
...
def _resize_to_fit_max_size(input_shape, max_size):
    ...
    scale_factor = tf.minimum(
        tf.cast(scale_factor, tf.float32), tf.cast(1.0, tf.float32)
    )
    return (
        _scale_dim(input_height, scale_factor),
        _scale_dim(input_width, scale_factor),
...
def resize_to_range_helper(input_shape, min_size, max_size=None, factor=None):
    ...
    if max_size:
        if factor:
            ...
            max_size = (
                max_size[0] - (max_size[0] - 1) % factor,
...
def resize_to_range(
    ...
    new_size = resize_to_range_helper(
        input_shape=(orig_height, orig_width),
        min_size=min_size,
        max_size=max_size,
        factor=factor,
    )
