...
class Evaluator(orbit.StandardEvaluator):
    ...
    def __init__(self, config, model, loss, global_step, model_dir):
        ...
        if common.TASK_DEPTH_AWARE_VIDEO_PANOPTIC_SEGMENTATION in self._supported_tasks:
    ...
    def eval_begin(self):
        ...
        if self._save_raw_predictions:
            ...
            if (
                common.TASK_DEPTH_AWARE_VIDEO_PANOPTIC_SEGMENTATION
    ...
    def eval_step(self, iterator):
        ...
        return tf.nest.map_structure(
            self._strategy.experimental_local_results, distributed_outputs
        )

    def _eval_step(self, inputs):
        tf.assert_equal(
            tf.shape(inputs[common.IMAGE])[0],
            1,
            "Currently only a "
            "batchsize of 1 is supported in evaluation due to resizing.",
        )
        outputs = self._model(inputs[common.IMAGE], training=False)
        raw_size = [inputs[common.GT_SIZE_RAW][0, 0], inputs[common.GT_SIZE_RAW][0, 1]]
        resized_size = [
        ...
        if self._decode_groundtruth_label:
            ...
            outputs = utils.undo_preprocessing(outputs, resized_size, raw_size)
            ...
            if (
                ...
                step_outputs[self._eval_depth_metric.name] = (
                    inputs[common.GT_DEPTH_RAW],
                    outputs[common.PRED_DEPTH_KEY],
                )
        else:
            # We only undo-preprocess for those defined in tuples in model/utils.py.
            outputs = utils.undo_preprocessing(outputs, resized_size, raw_size)
        # We only undo-preprocess for those defined in tuples in model/utils.py.
        inputs = utils.undo_preprocessing(inputs, resized_size, raw_size)
    ...
    def eval_reduce(self, state=None, step_outputs=None):
        ...
        if common.TASK_INSTANCE_SEGMENTATION in self._supported_tasks:
            ...
            for ap_result in zip(*tuple(step_outputs[self._eval_ap_metric.name])):
                ...
                batch_size = tf.shape(gt_panoptic)[0]
                for i in range(batch_size):
                    self._eval_ap_metric.update_state(
                        gt_panoptic[i],
                        pred_panoptic[i],
        ...
        if common.TASK_DEPTH_AWARE_VIDEO_PANOPTIC_SEGMENTATION in self._supported_tasks:
            ...
            for depth_result in zip(*tuple(step_outputs[self._eval_depth_metric.name])):
