...
def convert_policy(
    ...
    for n in range(num_policies):
        for i in range(2):
            operation_id, prob_id, magnitude = policy[
                6 * n + i * 3 : 6 * n + (i + 1) * 3
            ]
            policy_name = search_space[operation_id]
            policy_prob = augmentation_probabilities[prob_id] * probability_scale
            policy_list[n].append(
                (policy_name, policy_prob, magnitude * magnitude_scale)
            )
    return policy_list
...
available_policies = {
    ...
    "simple_classification_policy": convert_policy(
        simple_classification_policy, augmentation_reduced_operations, magnitude_scale=1
    ),
}
