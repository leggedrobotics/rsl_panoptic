...
def maybe_load_checkpoint(
    initial_checkpoint: Union[str, None], load_dict: Mapping[Any, Any]
) -> None:
    ...
    if _load_tf_model_garden_vision_checkpoint(initial_checkpoint):
        checkpoint = tf.train.Checkpoint(
            backbone=tf.train.Checkpoint(_encoder=load_dict["encoder"])
        )
    else:
...
def create_loss_metric_dict(loss_names, prefix="train_"):
    ...
    for loss_name in loss_names:
        loss_metric = tf.keras.metrics.Mean(prefix + loss_name, dtype=tf.float32)
        loss_metric_dict[loss_name] = loss_metric
    return loss_metric_dict


def check_if_variable_in_backbone(variable, encoder_name, encoder_variable_names):
