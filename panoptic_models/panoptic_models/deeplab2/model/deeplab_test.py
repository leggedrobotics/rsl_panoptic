...
def _create_model_from_test_proto(file_name, dataset_name="cityscapes_panoptic"):
    proto_filename = os.path.join(_CONFIG_PATH, file_name)
    config = _read_proto_file(proto_filename, config_pb2.ExperimentOptions())
    return (
        deeplab.DeepLab(config, dataset.MAP_NAME_TO_DATASET_INFO[dataset_name]),
        config,
    )
...
class DeeplabTest(tf.test.TestCase):
    ...
    def test_deeplab_with_panoptic_deeplab(self):
        ...
        expected_semantic_shape = [
            ...
            train_crop_size[0],
            train_crop_size[1],
            experiment_options.model_options.panoptic_deeplab.semantic_head.output_channels,
        ]
        expected_instance_center_shape = [2, train_crop_size[0], train_crop_size[1]]
    ...
    def test_deeplab_with_max_deeplab(self):
        ...
        expected_semantic_shape = [
            ...
            stride_4_size[0],
            stride_4_size[1],
            experiment_options.model_options.max_deeplab.auxiliary_semantic_head.output_channels,
        ]
        ...
        expected_pixel_space_normalized_feature_shape = [
            ...
            stride_4_size[0],
            stride_4_size[1],
            experiment_options.model_options.max_deeplab.pixel_space_head.output_channels,
        ]
        ...
        self.assertListEqual(
            resulting_dict[common.PRED_TRANSFORMER_CLASS_LOGITS_KEY].shape.as_list(),
            expected_transformer_class_logits_shape,
        )
        self.assertListEqual(
            resulting_dict[
    ...
    def test_deeplab_errors(self):
        ...
        experiment_options = _read_proto_file(
        ...
        with self.subTest("ResNet error."):
            with self.assertRaises(ValueError):
                ...
                _ = deeplab.DeepLab(
                    experiment_options, dataset.CITYSCAPES_PANOPTIC_INFORMATION
                )
