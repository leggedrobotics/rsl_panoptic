...
def assign_instances_to_previous_tracks(
    ...
    def body(
        i, current_centers_loop, prev_centers_loop, new_panoptic_map_loop, next_id_loop
    ):
        row_index = scores[i]
        i = tf.add(i, 1)
        ...
        if tf.shape(prev_centers_with_same_class)[0] > 0:
            ...
            distances = tf.reduce_sum(
                tf.square(prev_centers_with_same_class[:, :2] - center_location), axis=1
            )
            prev_center_index = tf.math.argmin(distances, axis=0, output_type=tf.int32)
            ...
            if min_dist < prev_centers_with_same_class[prev_center_index, 3]:
                ...
                new_panoptic_map_loop = tf.where(
                    center_mask, new_center_id, new_panoptic_map_loop
                )
                new_panoptic_map_loop.set_shape(shape)
                current_centers_loop = tf.tensor_scatter_nd_update(
                    current_centers_loop,
                    tf.expand_dims([row_index, 2], 0),
                    [new_center_id],
                )
            ...
            new_panoptic_map_loop.set_shape(shape)
            current_centers_loop = tf.tensor_scatter_nd_update(
                current_centers_loop, tf.expand_dims([row_index, 2], 0), [new_center_id]
            )
            next_id_loop += 1
            return (
                i,
    ...
    (_, current_centers, unmatched_centers, new_panoptic_map, next_id) = tf.while_loop(
        cond,
        body,
        (loop_start_index, current_centers, prev_centers, panoptic_map, next_id),
    )
    ...
    current_centers = tf.tensor_scatter_nd_add(
        current_centers, indices, tf.repeat([1], number_centers, axis=0)
    )
...
def render_panoptic_map_as_heatmap(
    panoptic_map: tf.Tensor, sigma: int, label_divisor: int, void_label: int
    ...
    for panoptic_id in unique_ids:
        ...
        mask_size = tf.reduce_max(mask_index, axis=1) - tf.reduce_min(
            mask_index, axis=1
        )
        ...
        bottom_right = (upper_left[0] + gaussian_size, upper_left[1] + gaussian_size)
