...
flags.DEFINE_string(
    "output_json_path", None, "Output path to which is written the image info JSON."
)
...
def _create_images_json(
    ...
    for city_dir in tf.io.gfile.listdir(image_dir):
        ...
        for image_basename in tf.io.gfile.listdir(os.path.join(image_dir, city_dir)):
