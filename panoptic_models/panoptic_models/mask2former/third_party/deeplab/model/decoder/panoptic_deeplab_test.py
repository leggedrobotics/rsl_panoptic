...
def _create_panoptic_deeplab_example_proto(num_classes=19):
    ...
    center_head = config_pb2.HeadOptions(output_channels=1, head_channels=32)
    regression_head = config_pb2.HeadOptions(output_channels=2, head_channels=32)
    ...
    panoptic_deeplab_options.low_level.add(feature_key="res3", channels_project=64)
    panoptic_deeplab_options.low_level.add(feature_key="res2", channels_project=32)
...
class PanopticDeeplabTest(tf.test.TestCase):
    ...
    def test_panoptic_deeplab_single_decoder_call_errors(self):
        ...
        with self.assertRaises(KeyError):
            input_dict = {
                "not_high": tf.random.uniform(shape=(2, 32, 32, 512)),
        ...
        with self.assertRaises(KeyError):
            input_dict = {
                "high": tf.random.uniform(shape=(2, 32, 32, 512)),
    ...
    def test_panoptic_deeplab_single_decoder_reset_pooling(self):
        ...
        self.assertTupleEqual(decoder._aspp._aspp_pool._pool_size, pool_size)
    ...
    def test_panoptic_deeplab_single_decoder_output_shape(self):
        ...
        input_shapes_list = [
            [[2, 128, 128, 128], [2, 256, 256, 64], [2, 32, 32, 512]],
            [[2, 129, 129, 128], [2, 257, 257, 64], [2, 33, 33, 512]],
        ]

        for shapes in input_shapes_list:
            input_dict = {
                "low_one": tf.random.uniform(shape=shapes[0]),
    ...
    def test_panoptic_deeplab_single_head_output_shape(self):
        ...
        for shape in input_shapes_list:
            ...
            self.assertListEqual(
                resulting_tensor["pred"].shape.as_list(), expected_shape
            )

    def test_panoptic_deeplab_decoder_output_shape(self):
        num_classes = 31
        model_options = _create_panoptic_deeplab_example_proto(num_classes=num_classes)
        ...
        input_shapes_list = [
            [[2, 256, 256, 64], [2, 128, 128, 128], [2, 32, 32, 512]],
            [[2, 257, 257, 64], [2, 129, 129, 128], [2, 33, 33, 512]],
        ]

        for shapes in input_shapes_list:
            input_dict = {
                "res2": tf.random.uniform(shape=shapes[0]),
    ...
    def test_panoptic_deeplab_sync_bn(self, strategy):
        ...
        model_options = _create_panoptic_deeplab_example_proto(num_classes=num_classes)
        input_dict = {
            "res2": tf.random.uniform(shape=[2, 257, 257, 64]),
    ...
    def test_panoptic_deeplab_decoder_ckpt_tems(self):
        ...
        self.assertIn(
            common.CKPT_INSTANCE_REGRESSION_HEAD_WITHOUT_LAST_LAYER, ckpt_dict
        )
        self.assertIn(common.CKPT_INSTANCE_REGRESSION_HEAD_LAST_LAYER, ckpt_dict)
        self.assertIn(common.CKPT_INSTANCE_CENTER_HEAD_WITHOUT_LAST_LAYER, ckpt_dict)
