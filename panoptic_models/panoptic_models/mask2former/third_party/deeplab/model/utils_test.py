...
class UtilsTest(tf.test.TestCase):
    ...
    def test_resize_logits(self):
        offset_logits = tf.convert_to_tensor(
            [
                [
                    [[2, 2], [2, 1], [2, 0]],
                    [[1, 2], [1, 1], [1, 0]],
        ...
        target_size = [5, 5]
        resized_logits = utils.resize_and_rescale_offsets(offset_logits, target_size)

        self.assertListEqual(resized_logits.shape.as_list(), [1, 5, 5, 2])
        for i in range(5):
            for j in range(5):
                np.testing.assert_array_almost_equal(
                    resized_logits.numpy()[0, i, j, :], [4 - i, 4 - j]
                )
    ...
    def test_zero_padding(self):
        ...
        padded_tensor_2 = utils.add_zero_padding(input_tensor_2, kernel_size=5, rank=3)
    ...
    def test_resize_function_error(self):
        ...
        with self.assertRaises(ValueError):
            _ = utils.resize_align_corners(
                input_tensor, [19, 19], method="not_a_valid_method"
            )
    ...
    def test_resize_function_invalid_rank(self):
        ...
        with self.assertRaisesRegex(ValueError, "should have rank of 4"):
    ...
    def test_resize_function_v1_compatibility(self):
        ...
        for shape, target_size, method in itertools.product(
            input_shapes, target_sizes, methods
        ):
            input_tensor = tf.random.uniform(shape=shape)

            result_tensor = utils.resize_align_corners(
                input_tensor, target_size, method
            )
            if method == "bilinear":
    ...
    def test_reshape_and_transpose_for_attention_operation(self):
        ...
        output = utils.reshape_and_transpose_for_attention_operation(
            images, num_heads=8
        )
    ...
    def test_get_stuff_class_ids(self):
        ...
        thing_class_ids = [3, 4]
        void_label_list = [5, 0]
        expected_stuff_class_ids_list = [[0, 1, 2], [1, 2, 5]]
        for void_label, expected_stuff_class_ids in zip(
            ...
            np.testing.assert_equal(stuff_class_ids, expected_stuff_class_ids)
