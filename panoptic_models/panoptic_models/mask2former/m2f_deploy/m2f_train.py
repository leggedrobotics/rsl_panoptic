...
Script has been adapted from panoptic_models.mask2former to include the registration process of the new datasets.
...
from panoptic_models.mask2former.m2f_deploy.utils.m2f_trainer import (
    DefaultTrainer,
    default_argument_parser,
)
...
def setup(args):
    ...
    cfg.MODEL.MAE_PRETRAIN = args.mae_pretrain  # TODO: Change to real cfg key in yaml
    cfg.MODEL.SAVE_BACKBONE = args.save_backbone_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--wb-entity",
        type=str,
        default="rsl_ssl_pan_seh",
        help="Entity name for Weights and Biases",
    )
    parser.add_argument(
        "--wb-name", type=str, default=None, help="Run name for Weights and Biases"
    )
    parser.add_argument(
        "--mae-pretrain", type=str, default=None, help="define mae pretrained model"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="output/default",
        help="Output directory for checkpoints and log data",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="data/dataset_v8",
        help="Path to dataset (expected folders: path/coco and path/segment), if None COCO_DST and SEGMENT_DST of config/params.py used",
    )
    parser.add_argument(
        "--caterpillar",
        type=str,
        default=None,  # adaption to specific dataset of the project
        help="Path to caterpillar dataset (for evaluation)",
    )
    parser.add_argument("--save-backbone-path", type=str, default=None)
    args = parser.parse_args()
    if not args.eval_only:
        wandb.init(project=args.wb_project, entity=args.wb_entity, name=args.wb_name)
