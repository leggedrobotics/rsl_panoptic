...
from panoptic_models.deeplab2.model.decoder import aspp
...
class PanopticDeepLabSingleDecoder(layers.Layer):
    ...
    def __init__(
        ...
        if len(low_level_feature_names) != len(low_level_channels_project):
            raise ValueError(
                "The Panoptic-DeepLab decoder requires the same number "
                "of low-level features as the number of low-level "
                "projection channels. But got %d and %d."
                % (len(low_level_feature_names), len(low_level_channels_project))
            )
        ...
        for i, channels_project in enumerate(low_level_channels_project):
            ...
            (
                current_low_level_conv_name,
                current_fusion_conv_name,
            ...
            utils.safe_setattr(
                self,
                current_fusion_conv_name,
                convolutions.StackedConv2DSame(
    ...
    def call(self, features, training=False):
        ...
        for i in range(len(self._low_level_feature_names)):
            ...
            combined_features = utils.resize_align_corners(
                combined_features, [target_h, target_w]
            )

            combined_features = tf.concat(
                [combined_features, low_level_features], self._channel_axis
            )
...
class PanopticDeepLab(layers.Layer):
    ...
    def __init__(
        ...
        if panoptic_deeplab_options.instance.enable:
            ...
            if panoptic_deeplab_options.instance.HasField("instance_decoder_override"):
                decoder_options = (
                    panoptic_deeplab_options.instance.instance_decoder_override
                )
