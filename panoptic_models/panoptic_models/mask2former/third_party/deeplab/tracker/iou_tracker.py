...
flags.DEFINE_string(
    "gt",
    None,
    "The path to the gt video frames. This folder "
    "should contain one folder per sequence.",
)
flags.DEFINE_string(
    "pred",
    None,
    "The path to the prediction video frames. "
    "This folder should contain one folder per sequence.",
)
flags.DEFINE_string(
    "output",
    "",
    "The path to store the tracked video frames."
    "This folder should contain one folder per sequence.",
)
...
flags.DEFINE_string(
    "dataset",
    "kitti_step",
    "The specified dataset is used"
    " to interpret the labels. Supported options are: " + ", ".join(dataset.MAP_NAMES),
)
flags.DEFINE_string(
    "optical_flow",
    None,
...
flags.DEFINE_integer(
    "input_channels",
    2,
    "DeepLab2 supports two formats when exporting "
...
class IoUTracker(object):
    ...
    def _match_instances_to_tracks(
        ...
        warped_instances: List[np.ndarray],
    ) -> np.ndarray:
    ...
    def update(
        ...
        predicted_occlusion: Optional[np.ndarray],
    ) -> np.ndarray:
        ...
        for class_index in self._classes_to_track:
            instances_mask = np.logical_and(
                predicted_classes == class_index, predicted_instances > 0
            )
                ...
                if predicted_occlusion is not None and predicted_flow is not None:
                    for instance in instances:
                        ...
                        warped_instances.append(
                            optical_flow_utils.remove_occlusions(
                                warped_instance, predicted_occlusion
                            )
                        )
        ...
        if self._next_track_id >= self._label_divisor:
            raise ValueError(
                "To many tracks were detected for the given "
    ...
    def _associate_instances_to_tracks(
        ...
        unmatched_instances = [
            inst_id
            for inst_id in range(number_of_instances)
            if inst_id not in matches_indices[:, 0]
        ]
        unmatched_tracks = [
            inst_id
            for inst_id in range(number_of_tracks)
...
def main(unused_args):
    if FLAGS.dataset not in dataset.MAP_NAME_TO_DATASET_INFO:
        raise ValueError(
            "Given dataset option is not a valid dataset. Please use "
    ...
    metric = stq.STQuality(
        num_classes, thing_classes, ignore_label, _LABEL_DIVISOR, 256 * 256 * 256
    )
        ...
        for gt_frame_path in sorted(
            ...
            if use_optical_flow:
                ...
                flow_path = os.path.join(
                    optical_flow_sequence_folder, "%06d%s" % (frame_id - 1, _FLOW_EXT)
                )
                occlusion_path = os.path.join(
                    optical_flow_sequence_folder,
            ...
            metric.update_state(
                tf.convert_to_tensor(gt_frame),
                tf.convert_to_tensor(pred_frame),
                sequence,
            )
