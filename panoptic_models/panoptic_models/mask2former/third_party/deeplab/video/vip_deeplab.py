...
class ViPDeepLab(tf.keras.Model):
    ...
    def _inference(
        self, input_tensor: tf.Tensor, next_input_tensor: tf.Tensor, training: bool
    ) -> Dict[Text, Any]:
        ...
        for eval_scale in self._eval_scales:
            ...
            next_scaled_images, _ = self._scale_images_and_pool_size(
    ...
    def call(self, input_tensor: tf.Tensor, training: bool = False) -> Dict[Text, Any]:
        ...
        if training:
            ...
            next_encoder_features = self._encoder(next_input_tensor, training=training)
            ...
            next_regression = tf.concat([next_regression_y, next_regression_x], axis=3)
