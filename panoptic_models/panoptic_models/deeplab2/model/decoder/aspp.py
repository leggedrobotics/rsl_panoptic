...
class ASPPPool(tf.keras.layers.Layer):
    ...
    def __init__(
        self, output_channels, name, bn_layer=tf.keras.layers.BatchNormalization
    ):
    ...
    def set_pool_size(self, pool_size):
        ...
        if None in pool_size and pool_size != (None, None):
            raise ValueError(
                "The ASPP pooling layer requires that the pooling size "
        ...
        logging.info(
            "Global average pooling in the ASPP pooling layer was replaced"
    ...
    def call(self, input_tensor, training=False):
        ...
        x = backend.pool2d(input_tensor, pool_size, padding="valid", pool_mode="avg")
...
class ASPP(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if not aspp_use_only_1x1_proj_conv:
            ...
            self._aspp_conv1 = ASPPConv(
                output_channels, rate1, name="aspp_conv1", bn_layer=bn_layer
            )
            self._aspp_conv2 = ASPPConv(
                output_channels, rate2, name="aspp_conv2", bn_layer=bn_layer
            )
            self._aspp_conv3 = ASPPConv(
                output_channels, rate3, name="aspp_conv3", bn_layer=bn_layer
            )
            self._aspp_pool = ASPPPool(
                output_channels, name="aspp_pool", bn_layer=bn_layer
            )
