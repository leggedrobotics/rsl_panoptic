...
def _find_closest_center_per_pixel(
    centers: tf.Tensor, center_offsets: tf.Tensor
) -> tf.Tensor:
...
def _get_instances_from_heatmap_and_offset(
    semantic_segmentation: tf.Tensor,
    center_heatmap: tf.Tensor,
    center_offsets: tf.Tensor,
    center_threshold: float,
    ...
    for thing_id in thing_class_ids:
        thing_segmentation = tf.where(
            tf.equal(semantic_segmentation, thing_id), 1, thing_segmentation
        )
    ...
    if tf.shape(centers)[0] == 0:
        return (
            tf.zeros_like(semantic_segmentation),
            processed_center_heatmap,
            tf.zeros_like(processed_center_heatmap),
        )

    instance_center_index = _find_closest_center_per_pixel(centers, center_offsets)
    ...
    instance_score_map = tf.reshape(instance_score_map, tf.shape(instance_segmentation))
    instance_score_map *= tf.cast(thing_segmentation, tf.float32)

    return (
        thing_segmentation * instance_segmentation,
        processed_center_heatmap,
        instance_score_map,
    )
...
def _get_panoptic_predictions(
    ...
    instance_map_lists = tf.TensorArray(tf.int32, size=batch_size, dynamic_size=False)
    center_map_lists = tf.TensorArray(tf.float32, size=batch_size, dynamic_size=False)
    ...
    for i in tf.range(batch_size):
        ...
        instance_score_map_lists = instance_score_map_lists.write(i, instance_score_map)
    ...
    if merge_semantic_and_instance_with_tf_op:
        ...
        from deeplab2.tensorflow_ops.python.ops import (
            merge_semantic_and_instance_maps_op as merge_ops,
        )
...
def _merge_semantic_and_instance_maps(
    ...
    panoptic_prediction = tf.ones_like(semantic_prediction) * void_label * label_divisor

    # Start to paste predicted `thing` regions to panoptic_prediction.
    # Infer `thing` segmentation regions from semantic prediction.
    semantic_thing_segmentation = tf.zeros_like(semantic_prediction, dtype=tf.bool)
    for thing_class in thing_class_ids:
        semantic_thing_segmentation = tf.math.logical_or(
            semantic_thing_segmentation, semantic_prediction == thing_class
        )
    ...
    for instance_id in instance_ids:
        ...
        if instance_id == 0:
            continue
        thing_mask = tf.math.logical_and(
            instance_maps == instance_id, semantic_thing_segmentation
        )
        ...
        semantic_majority = tf.cast(tf.math.argmax(semantic_bin_counts), tf.int32)
        ...
        new_instance_id = num_instance_per_semantic_label.read(semantic_majority) + 1
        num_instance_per_semantic_label = num_instance_per_semantic_label.write(
    ...
    for semantic_id in semantic_ids:
        if tf.reduce_sum(tf.cast(thing_class_ids == semantic_id, tf.int32)) > 0:
            continue
        # Check stuff area.
        stuff_mask = tf.math.logical_and(
            semantic_prediction == semantic_id, instance_stuff_regions
        )
