...
def _create_panoptic_deeplab_loss(dataset_info):
    ...
    regression_loss_options = trainer_pb2.LossOptions.SingleLossOptions(name="l1")
...
def _create_max_deeplab_loss(dataset_info):
    ...
    mask_id_cross_entropy_loss_options = trainer_pb2.LossOptions.SingleLossOptions()
    instance_discrimination_loss_options = trainer_pb2.LossOptions.SingleLossOptions()
    loss_options = trainer_pb2.LossOptions(
        semantic_loss=semantic_loss_options,
...
class RealDataEvaluatorTest(tf.test.TestCase):
    ...
    """
        config = text_format.Parse(
            experiment_options_textproto, config_pb2.ExperimentOptions()
        )

        model_proto_filename = os.path.join(
            _CONFIG_PATH, "example_coco_max_deeplab.textproto"
        )
        model_config = _read_proto_file(
            model_proto_filename, config_pb2.ExperimentOptions()
        )
...
class EvaluatorTest(tf.test.TestCase):
    ...
    """
        ...
        fake_datum = {
            common.IMAGE: tf.zeros([1, 1025, 2049, 3]),
            common.RESIZED_IMAGE: tf.zeros([1, 1025, 2049, 3]),
            common.GT_SIZE_RAW: tf.constant([[1025, 2049]], dtype=tf.int32),
            common.GT_SEMANTIC_KEY: tf.zeros([1, 1025, 2049], dtype=tf.int32),
            common.GT_SEMANTIC_RAW: tf.zeros([1, 1025, 2049], dtype=tf.int32),
            common.GT_PANOPTIC_RAW: tf.zeros([1, 1025, 2049], dtype=tf.int32),
            common.GT_IS_CROWD_RAW: tf.zeros([1, 1025, 2049], dtype=tf.uint8),
            common.GT_INSTANCE_CENTER_KEY: tf.zeros([1, 1025, 2049], dtype=tf.float32),
            common.GT_INSTANCE_REGRESSION_KEY: tf.zeros(
                [1, 1025, 2049, 2], dtype=tf.float32
            ),
            common.IMAGE_NAME: "fake",
            common.SEMANTIC_LOSS_WEIGHT_KEY: tf.zeros(
                [1, 1025, 2049], dtype=tf.float32
            ),
            common.CENTER_LOSS_WEIGHT_KEY: tf.zeros([1, 1025, 2049], dtype=tf.float32),
            common.REGRESSION_LOSS_WEIGHT_KEY: tf.zeros(
                [1, 1025, 2049], dtype=tf.float32
            ),
        }
