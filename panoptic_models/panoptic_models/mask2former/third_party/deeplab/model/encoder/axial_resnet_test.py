...
class AxialResNetTest(tf.test.TestCase):
    def test_axial_resnet_correct_output_shape(self):
        ...
        self.assertListEqual(
            endpoints["backbone_output"].get_shape().as_list(), [2, 5, 5, 2048]
        )
        self.assertListEqual(
            endpoints["transformer_class_feature"].get_shape().as_list(), [2, 128, 256]
        )
        self.assertListEqual(
            endpoints["transformer_mask_feature"].get_shape().as_list(), [2, 128, 256]
        )
        self.assertListEqual(
            endpoints["feature_panoptic"].get_shape().as_list(), [2, 17, 17, 256]
        )
        self.assertListEqual(
            endpoints["feature_semantic"].get_shape().as_list(), [2, 5, 5, 2048]
        )
        num_params = np.sum(
