...
def _create_max_deeplab_example_proto(num_non_void_classes=19):
    ...
    pixel_space_head = config_pb2.HeadOptions(output_channels=128, head_channels=256)
    ...
    max_deeplab_options.auxiliary_low_level.add(feature_key="res3", channels_project=64)
    max_deeplab_options.auxiliary_low_level.add(feature_key="res2", channels_project=32)
...
class MaXDeeplabTest(tf.test.TestCase):
    def test_max_deeplab_decoder_output_shape(self):
        ...
        input_dict = {
            "res2": tf.random.uniform([2, 17, 17, 256]),
            "res3": tf.random.uniform([2, 9, 9, 512]),
            "transformer_class_feature": tf.random.uniform([2, num_mask_slots, 256]),
            "transformer_mask_feature": tf.random.uniform([2, num_mask_slots, 256]),
            "feature_panoptic": tf.random.uniform([2, 17, 17, 256]),
            "feature_semantic": tf.random.uniform([2, 5, 5, 2048]),
        }
