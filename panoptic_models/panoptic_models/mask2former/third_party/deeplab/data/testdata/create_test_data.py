...
flags.DEFINE_string(
    ...
    "deeplab2/data/testdata/" "dummy_prediction.png",
    "Path to annotated test image with cityscapes encoding.",
)
flags.DEFINE_string(
    "panoptic_gt_output_path",
    "deeplab2/data/testdata/" "dummy_gt_for_vps.png",
...
def create_test_data(annotation_path):
    ...
    for label in unique_labels:
        ...
        if cityscapes_label == _CITYSCAPES_IGNORE:
            vps_panoptic[selected_pixels] = _CITYSCAPES_IGNORE * _PANOPTIC_LABEL_DIVISOR
            continue
        ...
        if cityscapes_id in segments_info:
            logging.info(
                "Merging segments with label %d into segment %d", label, cityscapes_id
            )
    ...
    return (
        cityscapes_panoptic.astype(np.uint8),
        vps_panoptic.astype(np.uint8),
...
def main(argv):
    ...
    json_annotation_path = os.path.join(
        FLAGS.output_cityscapes_root, "gtFine", "cityscapes_panoptic_dummy_trainId.json"
    )
    json_annotation_path = (
        json_annotation_path  # OSS: removed internal filename loading.
    )
