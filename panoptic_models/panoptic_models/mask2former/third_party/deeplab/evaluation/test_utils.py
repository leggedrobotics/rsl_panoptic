...
_TEST_DATA_DIR = "deeplab2/" "evaluation/testdata"


def read_test_image(
    testdata_path: str, image_format: Optional[str] = None
) -> np.ndarray:
    """Loads a test image.
...
def read_segmentation_with_rgb_color_map(
    ...
    if len(rgb_image.shape) != 3 or rgb_image.shape[2] != 3:
        raise AssertionError(
            "Expected RGB image, actual shape is %s" % (rgb_image.shape,)
        )
    ...
    output_dtype = output_dtype or type(next(iter(rgb_to_semantic_label.values())))
...
def panoptic_segmentation_with_class_map(
    instance_testdata_path: str, instance_label_to_semantic_label: Mapping[int, int]
    ...
    if set(np.unique(instance_labels)) != set(instance_label_to_semantic_label.keys()):
