...
def create_deeplab_model(
    config: config_pb2.ExperimentOptions, dataset_descriptor: dataset.DatasetDescriptor
) -> tf.keras.Model:
...
def run_experiment(
    ...
    logging.info(
        "Using strategy %s with %d replicas",
        type(strategy),
    ...
    if "eval" in mode:
        ...
        if (
            mode == "train_and_eval"
            and dataset_name != config.train_dataset_options.dataset
        ):
            raise ValueError("Using difference dataset_names in train_and_eval mode.")
        if num_gpus > 1:
            raise ValueError("Using more than one GPU for evaluation is not supported.")
    else:
    ...
    class_has_instances_list = dataset.MAP_NAME_TO_DATASET_INFO[
        dataset_name
    ...
    with strategy.scope():
        ...
        if "eval" in mode:
            evaluator = evaluator_lib.Evaluator(
                config, deeplab_model, losses, global_step, model_dir
            )
    ...
    if not config.model_options.restore_semantic_last_layer_from_initial_checkpoint:
        del init_dict[common.CKPT_SEMANTIC_LAST_LAYER]
    if not config.model_options.restore_instance_last_layer_from_initial_checkpoint:
