...
from panoptic_models.deeplab2.model.layers import axial_block_groups
from panoptic_models.deeplab2.model.layers import convolutions
from panoptic_models.deeplab2.model.layers import resized_fuse
from panoptic_models.deeplab2.model.layers import stems
...
class AxialResNet(tf.keras.Model):
    ...
    def __init__(
        ...
        # resnets.
        num_blocks_list = [3] + utils.scale_int_list(
        ...
        for index, stride in enumerate(total_strides_list):
            ...
            transformer_expansions_list.append(
                base_transformer_expansion * stride / 16.0
            )
        ...
        self._use_memory_feature = backbone_use_transformer_beyond_stride or (
            extra_decoder_use_transformer_beyond_stride and (not classification_mode)
        )
        if self._use_memory_feature:
            self._memory_feature_shape = (1, max_num_mask_slots, memory_channels)
            self._memory_feature_initializer = tf.keras.initializers.TruncatedNormal(
                stddev=1.0
            )
        ...
        while (
            current_stack < total_decoder_num_stacks
            or current_decoder_stride > high_resolution_output_stride
        ):
            decoder_stage += 1
            ...
            if (
                current_decoder_stride == output_stride
                and current_stack == backbone_decoder_num_stacks
            ):
        ...
        if (
            backbone_use_transformer_beyond_stride
            or extra_decoder_use_transformer_beyond_stride
        ):
    ...
    def build(self, input_shape):
        ...
        while (
            ...
            decoder_channels = int(
                round(original_resnet_input_stride * 64 * self._width_multiplier)
            )
            ...
            if (
                current_decoder_stride == self._output_stride
                and current_stack == self._backbone_decoder_num_stacks
            ):
                # Now that we have finished building the backbone, we either return the
                # classification endpoints, or continue building a non-backbone decoder
                # for panoptic segmentation.
    ...
    def call_encoder_before_stacked_decoder(self, inputs, training=False):
        ...
        if self._use_memory_feature:
            ...
            memory_feature = tf.tile(memory_feature, [tf.shape(inputs)[0], 1, 1])
        ...
        for index in range(self._first_block_index, 5):
            current_name = "_stage{}".format(index + 1)
            current_output, activated_output, memory_feature = getattr(
                self, current_name
    ...
    def call_stacked_decoder(
        ...
        while (
            ...
            decoder_features_list = [current_output] + stride_to_features[
                current_decoder_stride
            ][-2:]
    ...
    def call_extra_endpoints(
        ...
        if self._extra_decoder_num_stacks:
            endpoints["feature_semantic"] = high_resolution_outputs[
                self._backbone_decoder_num_stacks
            ]
        else:
    ...
    def call(self, inputs, training=False):
        ...
        if self._classification_mode:
            return endpoints
        endpoints = self.call_extra_endpoints(
            memory_feature,
            high_resolution_outputs,
            backbone_output,
            endpoints,
