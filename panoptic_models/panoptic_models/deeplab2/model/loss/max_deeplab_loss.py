...
from panoptic_seg.deeplab2 import common
from panoptic_seg.deeplab2 import config_pb2
from panoptic_seg.deeplab2.model import utils
from panoptic_seg.deeplab2.model.loss import base_loss
from panoptic_seg.deeplab2.model.loss import matchers_ops
...
def _generate_mask_slot_semantic_one_hot(
    ...
    indices = tf.concat(
        [batch_indices, matched_mask_slot_indices, semantic_indices], axis=-1
    )
    ...
    mask_slot_semantic_one_hot = tf.gather(
        mask_slot_semantic_one_hot, thing_stuff_tensor + 1, axis=2
    )
    return mask_slot_semantic_one_hot


def nonsquare_hungarian_matching(weights: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
...
def _mask_similarity(
    gt_mask: tf.Tensor, pred_mask: tf.Tensor, metric: str = "dice"
) -> tf.Tensor:
    ...
    if metric.lower() == "dice":
        denominator = (
            tf.expand_dims(tf.reduce_sum(gt_mask, axis=1), axis=2)
...
class MaXDeepLabLoss(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self._instance_discrimination_temperature = instance_discrimination_temperature
    ...
    def build(self, input_shapes: Tuple[Dict[Text, tf.Tensor], Dict[Text, tf.Tensor]]):
        ...
        self._mask_dice_term_modifier = self._focal_loss_alpha / self._num_mask_slots

        self._stuff_class_ids = utils.get_stuff_class_ids(
            self._num_thing_stuff_classes, self._thing_class_ids, self._ignore_label
        )
    ...
    def _pre_process_ground_truth(
        ...
        pixel_gt_semantic_map = utils.strided_downsample(
            y_true[common.GT_SEMANTIC_KEY], target_size=[output_height, output_width]
        )
        pixel_gt_semantic_map = tf.reshape(
            pixel_gt_semantic_map, [batch_size, output_height * output_width]
        )
        ...
        pixel_gt_non_void_mask = tf.ensure_shape(
            pixel_gt_non_void_mask, [batch_size, output_height * output_width]
        )
        ...
        pixel_gt_stuff_id_one_hot = tf.gather(
            pixel_gt_semantic_one_hot, self._stuff_class_ids, axis=-1
        )
        ...
        pixel_gt_thing_id_one_hot = tf.one_hot(
            pixel_gt_thing_id_map, self._max_thing_id
        )
        # Compute pixel_gt_mask_id_one_hot by concatenating thing masks with stuff
        # masks.
        pixel_gt_mask_id_one_hot = tf.concat(
            [pixel_gt_thing_id_one_hot, pixel_gt_stuff_id_one_hot], axis=-1
        )
        ...
        mask_gt_area_mask = tf.reshape(
            mask_gt_area > 0.5, [batch_size, self._pixel_gt_num_mask_id]
        )
        ...
        stuff_id_gt_semantic_map = tf.tile(
            tf.reshape(
                tf.cast(self._stuff_class_ids, tf.int32), [1, self._num_stuff_classes]
            ),
            [batch_size, 1],
        )
        ...
        mask_gt_semantic_map = (mask_gt_semantic_map + 1) * tf.cast(
            mask_gt_area_mask, tf.int32
        ) - 1
    ...
    def call(
        ...
        batch_size, output_height, output_width, _ = pixel_feature.get_shape().as_list()
        ...
        (
            ...
            mask_gt_semantic_one_hot,
            mask_gt_area,
        ) = self._pre_process_ground_truth(y_true, output_height, output_width)
        ...
        mask_average_feature = tf.einsum(
            "bpd,bpi->bid", pixel_feature, pixel_gt_mask_id_one_hot
        ) / tf.maximum(mask_gt_area, 1.0)
        ...
        instance_discrimination_similarity /= self._instance_discrimination_temperature
        mask_gt_non_void_mask_expanded_1 = tf.expand_dims(mask_gt_non_void_mask, axis=1)
        ...
        mask_gt_non_void_mask_expanded_2 = tf.expand_dims(mask_gt_non_void_mask, axis=2)
        ...
        full_permutation, nonsquare_permutation = nonsquare_hungarian_matching(
            hungarian_weights
        )

        # Extract the permutation (matching) between all existing non-void ground
        # truth masks and the matched predicted masks.
        matched_permutation = nonsquare_permutation * mask_gt_non_void_mask_expanded_2
        ...
        mask_slot_semantic_one_hot = _generate_mask_slot_semantic_one_hot(
            ...
            mask_gt_semantic_map,
            self._num_mask_slots,
            self._thing_stuff_class_ids,
        )

        # Compute the positive mask and the negative mask.
        mask_slot_positive_mask = tf.cast(
            tf.equal(tf.reduce_max(mask_slot_semantic_one_hot, axis=-1), 1.0),
            tf.float32,
        )
        ...
        mask_void_ratio = tf.stop_gradient(
            _mask_similarity(
        ...
        transformer_class_loss_weight = mask_slot_positive_mask * tf.maximum(
            matched_mask_dice, 1e-5
        ...
        transformer_class_one_hot = tf.concat(
            [
                ...
                tf.expand_dims(mask_slot_negative_mask, axis=-1),
            ],
            axis=-1,
        )
