...
def get_parser():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/coco/panoptic-segmentation/maskformer2_R50_bs1.yaml",
        metavar="FILE",
...
if __name__ == "__main__":
    ...
    if args.input_folder:
        assert os.path.isdir(
            args.input_folder
        ), f"The input path {args.input_folder} is not a directory"
        args.input = [
            entry.path for entry in os.scandir(args.input_folder) if entry.is_file()
        ]
    elif args.input:
        if len(args.input) == 1:
            args.input = glob.glob(os.path.expanduser(args.input[0]))
            assert args.input, f"The input path {args.input} was not found"
    else:
        raise ValueError("Either --input or --input_folder must be specified.")
