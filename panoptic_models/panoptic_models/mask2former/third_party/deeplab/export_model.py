...
_FLAGS_CKPT_PATH = flags.DEFINE_string(
    "checkpoint_path", default="", help="Path to the saved checkpoint."
)
...
class DeepLabModule(tf.Module):
    ...
    def __init__(
        ...
        self._is_motion_deeplab = (
            config.model_options.WhichOneof("meta_architecture") == "motion_deeplab"
        )
    ...
    def __call__(self, input_tensor: tf.Tensor) -> MutableMapping[Text, Any]:
        ...
        if self._is_motion_deeplab:
            ...
            (
                ...
                _,
                _,
            ) = self._preprocess_fn(image=image, prev_image=prev_image)
            processed_image = tf.concat([processed_image, processed_prev_image], axis=2)
        else:
        ...
        return utils.undo_preprocessing(outputs, resized_size, input_size)
...
def main(argv: Sequence[str]) -> None:
    ...
    tf.saved_model.save(module, _FLAGS_OUTPUT_PATH.value, signatures=signatures)
