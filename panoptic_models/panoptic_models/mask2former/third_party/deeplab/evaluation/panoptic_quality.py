...
class PanopticQuality(tf.keras.metrics.Metric):
    ...
    def compare_and_accumulate(
        ...
        ) * self.offset + tf.cast(pred_panoptic_label, tf.int64)
    ...
    def result(self) -> tf.Tensor:
        ...
        rq = tf.math.divide_no_nan(
            self.total_tp, self.total_tp + 0.5 * self.total_fn + 0.5 * self.total_fp
        )
        ...
        valid_classes = tf.not_equal(self.total_tp + self.total_fn + self.total_fp, 0)
