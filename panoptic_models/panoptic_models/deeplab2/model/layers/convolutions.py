...
def _compute_padding_size(kernel_size, atrous_rate):
    ...
    if pad_begin != pad_end:
        logging.warn(
            "Convolution requires one more padding to the "
    ...
    def squeeze_batch_dims(self, inp, op, inner_rank):
        ...
        with tf.name_scope("squeeze_batch_dims"):
            ...
            if isinstance(inner_shape, tf.TensorShape):
                ...
                inp_reshaped = tf.reshape(inp, tf.concat(([-1], inner_shape), axis=-1))
    ...
    def __init__(
        ...
        if use_bn:
            self._batch_norm = bn_layer(
                axis=3, name="batch_norm", gamma_initializer=bn_gamma_initializer
            )
...
class StackedConv2DSame(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        for index in range(num_layers):
            current_name = utils.get_conv_bn_act_current_name(index, use_bn, activation)
            utils.safe_setattr(
                self,
                current_name,
                convolution_op(
    ...
    def call(self, input_tensor, training=False):
        ...
        for index in range(self._num_layers):
            current_name = utils.get_conv_bn_act_current_name(
                index, self._use_bn, self._activation
            )
...
class Conv1D(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if use_bn:
            ...
            # last dimension.
            self._batch_norm = bn_layer(
                axis=2, name="batch_norm", gamma_initializer=bn_gamma_initializer
            )
