...
class EvaluatorTest(tf.test.TestCase):
    ...
    """
        ...
        result_dict = {
            common.PRED_SEMANTIC_PROBS_KEY: tf.zeros(
                [1, 1025, 2049, 19], dtype=tf.float32
            ),
            common.PRED_CENTER_HEATMAP_KEY: tf.zeros(
                [1, 1025, 2049, 1], dtype=tf.float32
            ),
            common.PRED_OFFSET_MAP_KEY: tf.zeros([1, 1025, 2049, 2], dtype=tf.float32),
        }
