...
flags.DEFINE_boolean(
    "create_panoptic_data", True, "Whether to create semantic or panoptic dataset."
)
...
_CLASS_HAS_INSTANCE_LIST = (
    dataset.CITYSCAPES_PANOPTIC_INFORMATION.class_has_instances_list
)
...
def _split_image_path(image_path):
    ...
    if image_folder != _FOLDERS_MAP["image"]:
        raise ValueError("Expects image path %s containing image folder." % image_path)

    pattern = "%s.%s" % (_POSTFIX_MAP["image"], _DATA_FORMAT_MAP["image"])
    if not file_name.endswith(pattern):
        raise ValueError("Image file name %s should end with %s" % (file_name, pattern))
...
def _get_panoptic_annotation(cityscapes_root, dataset_split, annotation_file_name):
    panoptic_folder = "cityscapes_panoptic_%s_trainId" % dataset_split
    return os.path.join(
        cityscapes_root, _FOLDERS_MAP["label"], panoptic_folder, annotation_file_name
    )
...
def _create_panoptic_label(image_path, segments_dict):
    ...
    panoptic_annotation_file = _get_panoptic_annotation(
        cityscapes_root, dataset_split, annotation_file_name
    )
...
def _convert_dataset(cityscapes_root, dataset_split, output_dir):
    ...
    if num_images != expected_dataset_size:
        raise ValueError(
            "Expects %d images, gets %d" % (expected_dataset_size, num_images)
        )
