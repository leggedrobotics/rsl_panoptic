...
class MaXDeepLabLossTest(tf.test.TestCase):
    def test_max_deeplab_loss(self):
        ...
        # Build the inputs.
        pred_dict = {
            common.PRED_PIXEL_SPACE_NORMALIZED_FEATURE_KEY: tf.random.uniform(
                shape=[2, 9, 9, 8]
            ),
            common.PRED_PIXEL_SPACE_MASK_LOGITS_KEY: tf.random.uniform(
                shape=[2, 9, 9, 128]
            ),
            common.PRED_TRANSFORMER_CLASS_LOGITS_KEY: tf.random.uniform(
                shape=[2, 128, 134]
            ),
        }
        gt_dict = {
            common.GT_SEMANTIC_KEY: tf.ones(shape=[2, 33, 33], dtype=tf.int32),
            common.GT_THING_ID_MASK_KEY: tf.ones(shape=[2, 33, 33], dtype=tf.int32),
        ...
        self.assertNotIn(common.INSTANCE_DISCRIMINATION_LOSS, loss_layer_2.loss_terms)
