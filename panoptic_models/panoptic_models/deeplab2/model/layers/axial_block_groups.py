...
from panoptic_models.deeplab2.model.layers import axial_blocks
from panoptic_models.deeplab2.model.layers import drop_path
from panoptic_models.deeplab2.model.layers import dual_path_transformer
from panoptic_models.deeplab2.model.layers import positional_encodings
from panoptic_models.deeplab2.model.layers import recompute_grad as recompute_grad_lib
...
class BlockGroup(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        for index in range(num_blocks):
            ...
            if (
                index == 0
                and original_resnet_input_stride * original_resnet_stride
                > output_stride
            ):
            ...
            if original_resnet_current_stride >= use_global_beyond_stride > 0:
                ...
                filters_list = [
                    filters * attention_bottleneck_expansion,
                    filters,
                    filters * 4,
                ]
                ...
                filters_list = [filters, filters, filters * 4]
                ...
                if original_resnet_input_stride * original_resnet_stride < 32:
                    ...
                    attention_type = None
                    recompute_grad = conv_use_recompute_grad or recompute_grad
                    filters_list = [filters * 4, filters * 4]
                else:
                    # Wider-ResNet uses an expanded bottleneck block in the last stage.
                    attention_type = None
                    recompute_grad = conv_use_recompute_grad or recompute_grad
                    filters_list = [filters, filters * 2, filters * 4]
            else:
    ...
    def call(self, inputs, training=False):
        ...
        for index in range(self._num_blocks):
            current_name, transformer_current_name = _get_current_names(index)
            block_fn_no_recompute = getattr(self, current_name)
            transformer_block_fn_no_recompute = getattr(self, transformer_current_name)
            ...
            if index == 0 and self._add_absolute_positional_encoding is not None:
                features = self._add_absolute_positional_encoding(
            ...
            if transformer_block_fn_no_recompute is not None:
                ...
                features = tf.reshape(features, [-1, height, width, channels])
                activated_features = tf.reshape(
                    activated_features, [-1, height, width, channels]
                )
