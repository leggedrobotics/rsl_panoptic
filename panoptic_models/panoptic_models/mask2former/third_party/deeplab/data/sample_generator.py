...
class PanopticSampleGenerator:
    ...
    def __init__(
        ...
        self._gaussian, self._gaussian_size = _compute_gaussian_from_std(self._sigma)
    ...
    def call(
        ...
        if next_image is not None:
            (
                ...
                image, label, prev_image=next_image, prev_label=next_label, depth=depth
            )
        else:
            (
                resized_image,
                ...
                image, label, prev_image=prev_image, prev_label=prev_label, depth=depth
            )
        ...
        if label is not None:
            ...
            if not self._only_semantic_annotations:
                ...
                sample[common.CENTER_LOSS_WEIGHT_KEY] = tf.squeeze(
                    tf.cast(non_crowd_and_non_ignore_regions, tf.float32), axis=2
                )
                ...
                sample[common.REGRESSION_LOSS_WEIGHT_KEY] = tf.squeeze(
                    tf.cast(non_crowd_things, tf.float32), axis=2
                )
                ...
                (
                    ...
                    next_offset,
                ) = self._generate_gt_center_and_offset(
                    panoptic_label,
                    semantic_weights,
                    prev_panoptic_label,
                    next_panoptic_label,
    ...
    def _generate_thing_id_mask_and_class(self, panoptic_label, non_crowd_things):
        ...
        for panoptic_id in unique_ids:
            ...
            # = 0)
            if (
                semantic_id == self._dataset_info["ignore_label"]
                or panoptic_id % self._dataset_info["panoptic_label_divisor"] == 0
            ):
                continue
            ...
            thing_id_mask = tf.where(
                panoptic_label == panoptic_id, thing_count, thing_id_mask
            )
    ...
    def _generate_prev_centers_with_noise(
        self,
        panoptic_label,
        ...
        for panoptic_id in unique_ids:
            ...
            bbox_size = tf.reduce_max(mask_index, axis=1) - tf.reduce_min(
                mask_index, axis=1
            )
            ...
            def add_center_gaussian(center_x_coord, center_y_coord, center):
                ...
                bottom_right = (
                    upper_left[0] + self._gaussian_size,
                ...
                indices = tf.transpose(
                    tf.stack([tf.reshape(indices_y, [-1]), tf.reshape(indices_x, [-1])])
                )
        ...
        center = center[
            center_pad_begin : (center_pad_begin + height),
    ...
    def _generate_gt_center_and_offset(
        self,
        panoptic_label,
        semantic_weights,
        prev_panoptic_label=None,
