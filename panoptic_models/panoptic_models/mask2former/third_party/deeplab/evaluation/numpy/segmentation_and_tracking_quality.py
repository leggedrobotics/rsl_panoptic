...
def _update_dict_stats(
    stat_dict: MutableMapping[int, np.ndarray], id_array: np.ndarray
):
...
class STQuality(object):
    ...
    def update_state(self, y_true: np.ndarray, y_pred: np.ndarray, sequence_id=0):
        ...
        if sequence_id in self._iou_confusion_matrix_per_sequence:
            idxs = (
                np.reshape(semantic_label, [-1]) << self._label_bit_shift
            ...
            np.add.at(
                self._iou_confusion_matrix_per_sequence[sequence_id], tuple(idxs), 1
            )
    ...
    def result(self) -> Mapping[Text, Any]:
        ...
        num_classes = np.count_nonzero(unions)
        ious = intersections.astype(np.double) / np.maximum(unions, _EPSILON).astype(
            np.double
        )
