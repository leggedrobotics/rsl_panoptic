...
class VideoPanopticPredictionStitcher(tf.keras.Model):
    ...
    def _increase_instance_ids_by_offset(self, panoptic: tf.Tensor) -> tf.Tensor:
        ...
        instance = tf.where(instance_mask, instance + self._overlap_offset, instance)
    ...
    def _compute_and_sort_iou_between_panoptic_ids(
        ...
        intersection_id_array = tf.cast(
            panoptic_1, tf.int64
        ) * self._combine_offset + tf.cast(panoptic_2, tf.int64)
    ...
    def _match_and_propagate_instance_ids(
        ...
        map_concat_to_next.insert(
            tf.cast(concat_panoptic_ids, tf.int32), tf.cast(next_panoptic_ids, tf.int32)
        )
        map_next_to_concat = tf.lookup.experimental.MutableHashTable(
            key_dtype=tf.int32, value_dtype=tf.int32, default_value=-1
        )
        map_next_to_concat.insert(
            tf.cast(next_panoptic_ids, tf.int32), tf.cast(concat_panoptic_ids, tf.int32)
        )
        (
            matched_concat_panoptic_ids,
            matched_next_panoptic_ids,
        ) = map_concat_to_next.export()
        returned_concat_panoptic_ids = map_next_to_concat.lookup(
            matched_next_panoptic_ids
        )
        matched_ids_mask = tf.equal(
            matched_concat_panoptic_ids, returned_concat_panoptic_ids
        )
    ...
    def call(self, concat_panoptic: tf.Tensor, next_panoptic: tf.Tensor) -> tf.Tensor:
