...
from panoptic_models.deeplab2.model.layers import axial_layers
from panoptic_models.deeplab2.model.layers import convolutions
from panoptic_models.deeplab2.model.layers import squeeze_and_excite
...
class AxialBlock(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if len(filters_list) == 3:
            ...
            if attention_type is None or attention_type.lower() == "none":
                ...
                if "double_global_attention" in axial_layer_config:
                    if axial_layer_config["double_global_attention"]:
                        raise ValueError(
                            "Double_global_attention takes no effect in "
                            "AxialAttention2D."
                        )
            ...
            raise ValueError(
                "Expect filters_list to have length 2 or 3; got %d" % len(filters_list)
            )
    ...
    def call(self, inputs):
        ...
        if len(self._filters_list) == 3:
            ...
            if self._attention_type is None or self._attention_type.lower() == "none":
