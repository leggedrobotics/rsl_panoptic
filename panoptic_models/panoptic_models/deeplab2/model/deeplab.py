...
from panoptic_models.deeplab2.model import builder
from panoptic_models.deeplab2.model import utils
from panoptic_models.deeplab2.model.post_processor import post_processor_builder
...
class DeepLab(tf.keras.Model):
    ...
    def __init__(
        ...
        pool_size = tuple(utils.scale_mutable_sequence(pool_size, 1.0 / output_stride))
        ...
        if self._is_max_deeplab and (
            ...
            raise ValueError("MaX-DeepLab does not support multi-scale inference yet.")

    def call(
        self, input_tensor: tf.Tensor, training: bool = False, adaptation: bool = False
    ):  # -> Dict[Text, Any]:
        ...
        if adaptation:
            result_dict = self._decoder(
                self._encoder(input_tensor, training=False), training=False
            )
            ...
            for eval_scale in self._eval_scales:
                ...
                scaled_images, scaled_pool_size = self._scale_images_and_pool_size(
                ...
                logging.info(
                    "Eval scale %s; setting pooling size to %s",
                    eval_scale,
                    scaled_pool_size,
                )
                ...
                if self._add_flipped_images:
                    pred_dict_reverse = self._decoder(
                        self._encoder(
                            tf.reverse(scaled_images, [2]), training=training
                        ),
                        training=training,
                    )
    ...
    def return_encoder_trainable_vars(self):
        return self._encoder.trainable_variables

    def return_encoder_bn_trainable_vars(self):
        vars_ = self._encoder.trainable_variables
        trainable_vars = []
        for var_ in vars_:
            if "beta" in var_.name or "gamma" in var_.name:
                trainable_vars.append(var_)
