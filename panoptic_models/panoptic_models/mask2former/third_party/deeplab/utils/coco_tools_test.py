...
class CocoToolsTest(absltest.TestCase):
    def testSingleImageDetectionMaskExport(self):
        ...
        for i, mask_annotation in enumerate(coco_annotations):
            self.assertEqual(
                mask_annotation["segmentation"]["counts"], expected_counts[i]
            )
            self.assertTrue(
                np.all(np.equal(mask.decode(mask_annotation["segmentation"]), masks[i]))
            )
    ...
    def testSingleImageGroundtruthExport(self):
        ...
        boxes = np.array(
            [[0, 0, 1, 1], [0, 0, 0.5, 0.5], [0.5, 0.5, 1, 1]], dtype=np.float32
        )
        coco_boxes = np.array(
            [[0, 0, 1, 1], [0, 0, 0.5, 0.5], [0.5, 0.5, 0.5, 0.5]], dtype=np.float32
        )
        classes = np.array([1, 2, 3], dtype=np.int32)
        ...
        for i, annotation in enumerate(coco_annotations):
            ...
            self.assertTrue(
                np.all(np.equal(mask.decode(annotation["segmentation"]), masks[i]))
            )
