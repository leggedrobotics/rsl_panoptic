...
class AxialAttention(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if not any(
            ...
            raise ValueError("Should use at least one similarity to compute attention.")
    ...
    def build(self, input_shape):
        ...
        if self._query_shape >= input_shape[1]:
            ...
            raise NotImplementedError(
                "Local axial attention has not been " "implemented yet."
            )
    ...
    def call(self, input_tensor, training=False):
        ...
        query = tf.reshape(
            query, [-1, self._query_shape, self._num_heads, self._key_depth_per_head]
        )
        ...
        retrieved_output = self._batch_norm_retrieved_output(
            retrieved_output, training=training
        )
...
class GlobalAttention2D(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if any(
            [
                kwargs.get("use_query_rpe_similarity", False),
                kwargs.get("use_key_rpe_similarity", False),
                kwargs.get("retrieve_value_rpe", False),
            ]
        ):
            raise ValueError(
                "GlobalAttention2D does not support relative positional " "encodings."
            )
