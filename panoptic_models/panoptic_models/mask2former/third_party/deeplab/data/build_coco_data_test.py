...
class BuildCOCODataTest(tf.test.TestCase):
    def setUp(self):
        ...
        image_path = os.path.join(
            self.data_dir, build_coco_data._FOLDERS_MAP[self.split]["image"]
        )
        panoptic_map_path = os.path.join(
            self.data_dir, build_coco_data._FOLDERS_MAP[self.split]["label"]
        )
        tf.io.gfile.makedirs(panoptic_map_path)
        panoptic_map_path = os.path.join(
            panoptic_map_path, "panoptic_%s2017" % self.split
        )
    ...
    def _create_image_and_panoptic_map(self, image_path, panoptic_path, image_id):
        ...
        with tf.io.gfile.GFile(
            os.path.join(image_path, "%012d.jpg" % image_id), "wb"
        ) as f:
            Image.fromarray(image).save(f, format="JPEG")

        # Dummy panoptic map.
        semantic = np.random.randint(0, 201, (self.height, self.width), dtype=np.int32)
        instance_ = np.random.randint(0, 100, (self.height, self.width), dtype=np.int32)
        ...
        with tf.io.gfile.GFile(
            os.path.join(panoptic_path, "%012d.png" % image_id), "wb"
        ) as f:
    ...
    def test_build_coco_dataset_correct(self):
        ...
        output_record = os.path.join(
            FLAGS.test_tmpdir,
            "%s-%05d-of-%05d.tfrecord" % (self.split, 0, build_coco_data._NUM_SHARDS),
        )
