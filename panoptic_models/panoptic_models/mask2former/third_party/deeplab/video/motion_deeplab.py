...
class MotionDeepLab(tf.keras.Model):
    ...
    def call(self, input_tensor: tf.Tensor, training=False) -> Dict[Text, Any]:
        ...
        for key, value in pred.items():
            if (
                key == common.PRED_OFFSET_MAP_KEY
                or key == common.PRED_FRAME_OFFSET_MAP_KEY
            ):
                result_dict[key] = utils.resize_and_rescale_offsets(
                    value, [input_h, input_w]
                )
            else:
                result_dict[key] = utils.resize_bilinear(value, [input_h, input_w])
    ...
    def _add_previous_heatmap_to_input(self, input_tensor: tf.Tensor) -> tf.Tensor:
