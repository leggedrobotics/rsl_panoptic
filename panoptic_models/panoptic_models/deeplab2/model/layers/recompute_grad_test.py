...
class RecomputeGradTest(tf.test.TestCase):
    def test_real_deeplab_models(self):
        ...
        def function():
            ...
            outs, grads = _compute_deeplab_gradients(test_input, model, True)
        ...
        # Similar outputs.
        self.assertAllClose(
            outs.values[0], outs_recompute.values[0], rtol=1e-4, atol=1e-4
        )
        ...
        for grad, grad_recompute in zip(grads, grads_recompute):
            ...
            if isinstance(grad, tf.IndexedSlices) and isinstance(
                grad_recompute, tf.IndexedSlices
            ):
                continue
...
def _make_gradients_op(model, x):
    ...
    return (
        tf.function(experimental_compile=True)(lambda: f(x))
        if tf.executing_eagerly()
        else tf.compat.v1.tpu.rewrite(f, (x,))
    )
    ...
    def test_xla_model_correctness(self):
        ...
        with tf.device(self.device):
            ...
            for source, target in zip(
                control_model.trainable_variables, recompute_model.trainable_variables
            ):
