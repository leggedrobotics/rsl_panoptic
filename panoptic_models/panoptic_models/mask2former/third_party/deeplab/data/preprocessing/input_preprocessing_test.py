...
class InputPreprocessingTest(tf.test.TestCase):
    ...
    def test_cropping(self):
        ...
        self.assertListEqual(
            processed_image.shape.as_list(), [crop_height, crop_width, 3]
        )
        self.assertListEqual(
            processed_label.shape.as_list(), [crop_height, crop_width, 1]
        )
    ...
    def test_die_on_target_size_greater_than_image_size(self):
        ...
        with self.assertRaises(tf.errors.InvalidArgumentError):
            _ = input_preprocessing._pad_image_and_label(
                self._image, None, 0, 0, height, width - 1
            )

        with self.assertRaises(tf.errors.InvalidArgumentError):
            _ = input_preprocessing._pad_image_and_label(
                self._image, None, 0, 0, height - 1, width
            )
    ...
    def test_die_if_target_size_not_possible_with_given_offset(self):
        ...
        with self.assertRaises(tf.errors.InvalidArgumentError):
            _ = input_preprocessing._pad_image_and_label(
                self._image, None, 3, 3, height + 2, width + 2
            )
    ...
    def test_set_min_resize_value_only_during_training(self):
        ...
        _, processed_image, _, _, _, _ = input_preprocessing.preprocess_image_and_label(
            image=self._image,
            label=self._label,
            crop_height=crop_height,
            crop_width=crop_width,
