...
from panoptic_models.deeplab2.model.layers import activations
...
class AttentionOperation(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        retrieved_value = tf.einsum("bhlm,bmhd->bhld", attention_weights, value)
        ...
        # Reshape the output.
        return utils.transpose_and_reshape_for_attention_operation(retrieved_value)
...
class DualPathTransformerLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self._memory_attention = AttentionOperation(
            "memory_attention", activation, transformer_activation, bn_layer=bn_layer
        )
        if use_pixel2memory_feedback_attention:
            self._pixel_attention = AttentionOperation(
                "pixel_attention", activation, transformer_activation, bn_layer=bn_layer
            )

        self._use_memory_self_attention = use_memory_self_attention
        self._use_pixel2memory_feedback_attention = use_pixel2memory_feedback_attention
    ...
    def call(self, inputs):
        ...
        memory_space = self._memory_conv1_bn_act(memory_space_input, training=training)
        ...
        if self._use_memory_self_attention or self._use_pixel2memory_feedback_attention:
            memory_space_qkv = self._memory_qkv_conv_bn(memory_space, training=training)
            # Split, reshape, and transpose the query, key, and value.
            memory_query, memory_key, memory_value = tf.split(
                memory_space_qkv,
        ...
        if self._use_pixel2memory_feedback_attention:
            pixel_space_qkv = self._pixel_qkv_conv_bn(pixel_space, training=training)
            ...
            pixel_key, pixel_value = tf.split(
                pixel_space_kv,
                [self._total_key_depth, self._total_value_depth],
                axis=-1,
            )
        ...
        memory_space_output = self._activation_fn(memory_space_input + memory_space)

        # Apply an optional feed-forward network to the memory space.
        if self._feed_forward_network_channels > 0:
            memory_space = self._memory_ffn_conv1_bn_act(
                memory_space_output, training=training
            )
            memory_space = self._memory_ffn_conv2_bn(memory_space, training=training)
            if memory_space_feed_forward_network_drop_path_mask is not None:
                memory_space = (
                    memory_space * memory_space_feed_forward_network_drop_path_mask
                )
        ...
        return (pixel_space_output, activated_pixel_space_output, memory_space_output)
