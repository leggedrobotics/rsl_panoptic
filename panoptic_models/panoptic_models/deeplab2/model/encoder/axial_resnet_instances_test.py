...
class AxialResnetInstancesTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_model_output_shape_and_num_params(
        ...
        if model_name in ("axial_resnet_s", "axial_deeplab_s"):
            self.assertListEqual(
                output["res5"].get_shape().as_list(), [None, 14, 14, 1024]
            )
        else:
            self.assertListEqual(
                output["res5"].get_shape().as_list(), [None, 14, 14, 2048]
            )
        num_params = np.sum(
    ...
    def test_max_deeplab_s_output_shape_and_num_params(self):
        ...
        self.assertListEqual(
            endpoints["backbone_output"].get_shape().as_list(), [None, 5, 5, 2048]
        )
        self.assertListEqual(
        ...
        self.assertListEqual(
            endpoints["feature_panoptic"].get_shape().as_list(), [None, 17, 17, 256]
        )
        self.assertListEqual(
            endpoints["feature_semantic"].get_shape().as_list(), [None, 5, 5, 2048]
        )
        num_params = np.sum(
            [np.prod(v.get_shape().as_list()) for v in model.trainable_weights]
        )
    ...
    def test_max_deeplab_l_output_shape_and_num_params(self):
        ...
        self.assertListEqual(
            endpoints["feature_semantic"].get_shape().as_list(), [None, 17, 17, 256]
        )
        num_params = np.sum(
            [np.prod(v.get_shape().as_list()) for v in model.trainable_weights]
        )
    ...
    def test_global_attention_absolute_positional_encoding_names(self):
        ...
        test_variable_name2 = (
            "max_deeplab_s_backbone/stage4/block2/attention/global/qkv_kernel:0"
        )
