...
def compute_average_top_k_loss(loss: tf.Tensor, top_k_percentage: float) -> tf.Tensor:
    ...
    if top_k_percentage != 1.0:
        ...
        top_k_pixels = tf.cast(
            tf.math.round(
                top_k_percentage * tf.cast(num_elements_per_sample, tf.float32)
            ),
            tf.int32,
        )
...
def compute_mask_dice_loss(
    y_true: tf.Tensor, y_pred: tf.Tensor, prediction_activation="softmax"
) -> tf.Tensor:
    ...
    y_pred_flat = tf.reshape(y_pred, [batch, -1, channels]) * valid_flat
    ...
    denominator = (
        tf.reduce_sum(y_pred_flat, axis=1) + tf.reduce_sum(y_true_flat, axis=1) + smooth
    )
    loss = 1.0 - tf.math.divide_no_nan(intersection, denominator)
    return loss


def mean_absolute_error(
    y_true: tf.Tensor, y_pred: tf.Tensor, force_keep_dims=False
) -> tf.Tensor:
...
def encode_one_hot(
    gt: tf.Tensor, num_classes: int, weights: tf.Tensor, ignore_label: Optional[int]
):
...
def is_one_hot(gt: tf.Tensor, pred: tf.Tensor):
    ...
    return (
        len(gt_shape) == len(pred_shape)
        and gt_shape[0] == pred_shape[0]
        and gt_shape[-1] == pred_shape[-1]
    )
...
def _ensure_topk_value_is_percentage(top_k_percentage: float):
    ...
    if top_k_percentage < 0.0 or top_k_percentage > 1.0:
        raise ValueError(
            "The top-k percentage parameter must lie within 0.0 and "
...
class TopKGeneralLoss(tf.keras.losses.Loss):
    ...
    def __init__(
        ...
        super(TopKGeneralLoss, self).__init__(reduction=tf.keras.losses.Reduction.NONE)
    ...
    def call(
        self, y_true: Dict[Text, tf.Tensor], y_pred: Dict[Text, tf.Tensor]
    ) -> tf.Tensor:
        ...
        return compute_average_top_k_loss(per_pixel_loss, self._top_k_percent_pixels)
...
class TopKCrossEntropyLoss(tf.keras.losses.Loss):
    ...
    def call(
        ...
        if is_one_hot(gt, pred):
            ...
            gt, weights = encode_one_hot(
                gt, self._num_classes, weights, self._ignore_label
            )
        ...
        return compute_average_top_k_loss(
            weighted_pixel_losses, self._top_k_percent_pixels
        )
...
class MaskDiceLoss(tf.keras.losses.Loss):
    ...
    def call(
        ...
        weighted_dice_losses = tf.multiply(
            compute_mask_dice_loss(gt, pred, self._prediction_activation), weights
        )
...
class SILogError(tf.keras.losses.Loss):
    ...
    def __init__(self, gt_key: Text, pred_key: Text, ignore_label: float):
        # Implicit reduction might mess with tf.distribute.Strategy, hence we
        # explicitly reduce the loss.
    ...
    def call(
        ...
        def _compute_error(loss_input):
            ...
            silog_error = tf.reduce_mean(tf.square(gt_log - pred_log)) - tf.square(
                tf.reduce_mean(gt_log - pred_log)
            )
            return silog_error
