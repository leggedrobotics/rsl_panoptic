...
class DSTQuality(stq.STQuality):
    ...
    def __init__(
        ...
        if not (
            isinstance(depth_threshold, tuple) or isinstance(depth_threshold, list)
        ):
    ...
    def update_state(
        ...
        inlier_error = tf.maximum(
            d_valid_pred / d_valid_true, d_valid_true / d_valid_pred
        )
        ...
        for threshold_index, threshold in enumerate(self._depth_threshold):
            ...
            inlier_counts[sequence_id] = inlier_counts.get(sequence_id, 0) + int(
                num_inliers.numpy()
            )
        # Update the total counts of the depth labels.
        self._depth_total_counts[sequence_id] = self._depth_total_counts.get(
            sequence_id, 0
    ...
    def result(self):
        ...
        for threshold_index, threshold in enumerate(self._depth_threshold):
            ...
            for index, sequence_id in enumerate(self._ground_truth):
                ...
                if sequence_total > 0:
                    dq_per_seq_at_threshold[threshold][index] = (
                        sequence_inlier / sequence_total
                    )
        ...
        for _, threshold in enumerate(self._depth_threshold):
            ...
            dq_results["DQ_per_seq@{}".format(threshold)] = dq_per_seq_at_threshold[
                threshold
            ]
        ...
        for _, threshold in enumerate(self._depth_threshold):
            dstq_results["DSTQ@{}".format(threshold)] = (
                stq_results["STQ"] ** 2 * dq_at_threshold[threshold]
            ) ** (1 / 3)
            dstq_results["DSTQ_per_seq@{}".format(threshold)] = [
                (stq_result**2 * dq_result) ** (1 / 3)
                for stq_result, dq_result in zip(
