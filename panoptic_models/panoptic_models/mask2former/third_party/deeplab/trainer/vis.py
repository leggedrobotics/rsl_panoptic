...
def _convert_train_id_to_eval_id(
    prediction: np.ndarray, dataset_name: str
) -> np.ndarray:
    ...
    if "cityscapes" in dataset_name:
        ...
        raise ValueError(
            "Unsupported dataset %s for converting semantic class IDs." % dataset_name
        )
    length = np.maximum(256, len(train_id_to_eval_id))
    to_eval_id_map = np.zeros((length), dtype=prediction.dtype)
    dataset_ids = np.asarray(train_id_to_eval_id, dtype=prediction.dtype)
...
def store_raw_predictions(
    ...
    vis_utils.save_annotation(
        semantic_prediction, output_folder, image_filename, add_colormap=False
    )
    ...
    for pred_panoptic_key in pred_panoptic_keys:
        ...
        predicted_instance_labels = (
            predictions[pred_panoptic_key] % dataset_info.panoptic_label_divisor
        )
        ...
        if raw_panoptic_format == "two_channel_png":
            if np.max(predicted_semantic_labels) > 255:
                raise ValueError(
                    "Overflow: Semantic IDs greater 255 are not supported "
            ...
            vis_utils.save_annotation(
                panoptic_outputs, output_folder, panoptic_filename, add_colormap=False
            )
            ...
            with tf.io.gfile.GFile(
                os.path.join(output_folder, panoptic_filename + ".npy"), "w"
            ) as f:
                np.save(f, panoptic_outputs)
        else:
            raise ValueError("Unknown raw_panoptic_format %s." % raw_panoptic_format)
...
def store_predictions(
    ...
    vis_utils.save_annotation(
        image, save_dir, _IMAGE_FORMAT % image_id, add_colormap=False
    )
    ...
    if common.PRED_CENTER_HEATMAP_KEY in predictions:
        ...
        vis_utils.save_annotation(
            vis_utils.overlay_heatmap_on_image(heatmap_pred, image),
            save_dir,
            _CENTER_HEATMAP_PREDICTION_FORMAT % image_id,
            add_colormap=False,
        )
        vis_utils.save_annotation(
            vis_utils.overlay_heatmap_on_image(heat_map_gt, image),
            save_dir,
    ...
    if common.PRED_OFFSET_MAP_KEY in predictions:
        ...
        center_offset_prediction_rgb = vis_utils.flow_to_color(center_offset_prediction)
        semantic_prediction = predictions[common.PRED_SEMANTIC_KEY]
    ...
    error_prediction = (
        ...
        & (label != dataset_info.ignore_label)
    ).astype(np.uint8) * 255
    vis_utils.save_annotation(
        error_prediction, save_dir, _ANALYSIS_FORMAT % (image_id), add_colormap=False
    )
