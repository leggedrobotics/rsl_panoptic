...
class BlocksTest(tf.test.TestCase):
    def test_inverted_bottleneck_block_output_shape(self):
        ...
        output_channels = 6
        input_tensor = tf.random.uniform(shape=(batch, height, width, input_channels))
        ...
        self.assertListEqual(
            output_tensor.get_shape().as_list(), [batch, height, width, output_channels]
        )
    ...
    def test_inverted_bottleneck_block_feature_map_alignment(self):
        ...
        self.assertAllClose(
            ivb_block1.get_weights(), ivb_block2.get_weights(), atol=1e-4, rtol=1e-4
        )
