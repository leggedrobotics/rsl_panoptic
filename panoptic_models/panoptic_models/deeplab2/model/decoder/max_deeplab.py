...
def _get_transformer_class_head_num_classes(
    auxiliary_semantic_head_output_channels, ignore_label
):
...
class MaXDeepLab(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        low_level_channels_project = [
            item.channels_project for item in max_deeplab_options.auxiliary_low_level
        ]
        ...
        self._pixel_space_feature_batch_norm = bn_layer(
            axis=-1,
            name="pixel_space_feature_batch_norm",
        ...
        self._pixel_space_mask_batch_norm = bn_layer(
            axis=-1,
            name="pixel_space_mask_batch_norm",
    ...
    def checkpoint_items(self):
        items = {
            ...
            common.CKPT_SEMANTIC_HEAD_WITHOUT_LAST_LAYER: self._auxiliary_semantic_head.conv_block,
            common.CKPT_SEMANTIC_LAST_LAYER: self._auxiliary_semantic_head.final_conv,
            ...
            common.CKPT_PIXEL_SPACE_FEATURE_BATCH_NORM: self._pixel_space_feature_batch_norm,
            common.CKPT_PIXEL_SPACE_MASK_BATCH_NORM: self._pixel_space_mask_batch_norm,
        }
        return items
    ...
    def call(self, features, training=False):
        ...
        pixel_space_feature = self._pixel_space_head(
            panoptic_features, training=training
        )[_PIXEL_SPACE_FEATURE_KEY]
        pixel_space_feature = self._pixel_space_feature_batch_norm(pixel_space_feature)
        ...
        transformer_class_logits = add_bias_towards_void(transformer_class_logits)
        results[common.PRED_TRANSFORMER_CLASS_LOGITS_KEY] = transformer_class_logits

        # Transformer mask kernel.
        transformer_mask_kernel = self._transformer_mask_head(transformer_mask_feature)
        ...
        pixel_space_mask_logits = tf.einsum(
            "bhwd,bid->bhwi", pixel_space_normalized_feature, transformer_mask_kernel
        )
        ...
        results[common.PRED_PIXEL_SPACE_MASK_LOGITS_KEY] = pixel_space_mask_logits
