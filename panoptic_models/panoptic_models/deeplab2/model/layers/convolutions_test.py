...
class ConvolutionsTest(tf.test.TestCase):
    ...
    def test_conv2dsame_conv(self):
        conv = convolutions.Conv2DSame(
            output_channels=1, kernel_size=1, name="conv", use_bn=False, activation=None
        )
        input_tensor = tf.random.uniform(shape=(2, 180, 180, 5))

        predicted_tensor = conv(input_tensor)
        expected_tensor = np.dot(input_tensor.numpy(), conv._conv.get_weights()[0])[
            ..., 0, 0
        ]
    ...
    def test_conv2dsame_relu6(self):
        ...
        input_tensor = tf.random.uniform(shape=(2, 180, 180, 5)) * 10.0
    ...
    def test_depthwise_conv(self):
        conv = convolutions.DepthwiseConv2DSame(
            kernel_size=1, use_bn=False, use_bias=True, activation=None, name="conv"
        )
        input_tensor = tf.random.uniform(shape=(2, 180, 180, 5))
    ...
    def test_depthwise_shape(self):
        conv = convolutions.DepthwiseConv2DSame(
            kernel_size=3, use_bn=True, use_bias=False, activation="relu", name="conv"
        )
    ...
    def test_global_context(self):
        ...
        np.testing.assert_array_almost_equal(
            input_tensor.numpy(), output_tensor.numpy()
        )
    ...
    def test_switchable_atrous_conv_class(self):
        ...
        sac_layer = convolutions.SwitchableAtrousConvolution(
            64, kernel_size=3, padding="same", name="sac_conv"
        )
        ...
        switch_conv.bias = tf.random.uniform(
            switch_conv.bias.shape, minval=-1, maxval=1, dtype=switch_conv.bias.dtype
        )
        small_conv = tf.keras.layers.Conv2D(
            64, kernel_size=3, padding="same", dilation_rate=1, name="small_conv"
        )
        large_conv = tf.keras.layers.Conv2D(
            64, kernel_size=3, padding="same", dilation_rate=3, name="large_conv"
        )
        ...
        expected_outputs = (
            switch_outputs * large_outputs + (1 - switch_outputs) * small_outputs
        )
        np.testing.assert_array_almost_equal(expected_outputs.numpy(), outputs.numpy())
    ...
    def test_switchable_atrous_conv_in_conv2dsame(self):
        ...
        np.testing.assert_array_almost_equal(
            output_tensor.shape.as_list(), [3, 180, 180, 64]
        )

    def test_conv1d_shape(self):
        conv = convolutions.Conv1D(
            output_channels=64, name="conv", use_bias=False, use_bn=True
        )
    ...
    def test_separable_conv2d_same_output_shape(self):
        conv = convolutions.SeparableConv2DSame(
            output_channels=64, kernel_size=3, name="conv"
        )
