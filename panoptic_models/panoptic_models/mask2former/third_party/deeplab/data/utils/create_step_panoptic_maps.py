...
flags.DEFINE_string(
    "input_instance_root_dir", None, "Path to a directory containing the instance map."
)
flags.DEFINE_string(
    "output_panoptic_map_root_dir",
    None,
...
flags.DEFINE_integer(
    "kernel_size",
    15,
    "Kernel size to extend instance object boundary when "
...
def encode_panoptic_map(panoptic_map: np.ndarray) -> np.ndarray:
    ...
    return np.dstack((semantic_id, instance_id // 256, instance_id % 256)).astype(
        np.uint8
    )
...
def _compute_panoptic_id(
    semantic_id: Union[int, np.ndarray], instance_id: Union[int, np.ndarray]
) -> Any:
...
def _remap_motchallege_semantic_indices(panoptic_id: np.ndarray) -> np.ndarray:
    ...
    for i, label in enumerate(sorted(all_labels - set(MOTCHALLENGE_MERGED_CLASSES))):
        semantic_id[semantic_id == label] = i
    return _compute_panoptic_id(semantic_id, instance_id)


def _get_semantic_maps(
    semantic_map_root: str, dataset_split: str, sequence_id: str
) -> Sequence[str]:
...
class StepPanopticMapGenerator(object):
    ...
    def _update_semantic_label_map(
        self, instance_map: np.ndarray, semantic_map: np.ndarray
    ) -> np.ndarray:
        ...
        for label in (SEMANTIC_CAR, SEMANTIC_PERSON):
            ...
            void_boundary = np.logical_and(
                dilated_instance_mask - instance_mask, semantic_mask
            )
            updated_semantic_map[void_boundary] = SEMANTIC_VOID
        return updated_semantic_map

    def merge_panoptic_map(
        self, semantic_map: np.ndarray, instance_map: np.ndarray
    ) -> np.ndarray:
        ...
        panoptic_map[mask_car] = _compute_panoptic_id(
            SEMANTIC_CAR, instance_id.astype(np.int32)
        )
    ...
    def build_panoptic_maps(
        self,
        semantic_map_root: str,
        instance_map_root: str,
        dataset_split: str,
        sequence_id: str,
        ...
        Args:
          ...
          panoptic_map_root: Panoptic map root folder where the encoded panoptic
            maps will be saved.
        """
        semantic_maps = _get_semantic_maps(
            semantic_map_root, dataset_split, sequence_id
        )
        for semantic_map_path in semantic_maps:
            image_name = os.path.basename(semantic_map_path)
            instance_map_path = os.path.join(
                instance_map_root, dataset_split, sequence_id, image_name
            )
            ...
            encoded_panoptic_map = Image.fromarray(
                encode_panoptic_map(panoptic_map)
            ).convert("RGB")
            panoptic_map_path = os.path.join(
                panoptic_map_root, dataset_split, sequence_id, image_name
            )
...
def main(argv: Sequence[str]) -> None:
    ...
    panoptic_map_generator = StepPanopticMapGenerator(
        FLAGS.kernel_size, FLAGS.dataset_name
    )
    for dataset_split in ("train", "val", "test"):
        ...
        for set_dir in tf.io.gfile.listdir(sem_dir):
            tf.io.gfile.makedirs(
                os.path.join(FLAGS.output_panoptic_map_root_dir, dataset_split, set_dir)
            )
            logging.info(
                "Start to create panoptic map for split %s, sequence %s.",
                dataset_split,
                set_dir,
            )
            panoptic_map_generator.build_panoptic_maps(
                FLAGS.input_semantic_map_root_dir,
                FLAGS.input_instance_root_dir,
                dataset_split,
                set_dir,
                FLAGS.output_panoptic_map_root_dir,
            )
