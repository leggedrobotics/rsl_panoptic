...
class PanopticQualityMetricTest(tf.test.TestCase):
    def test_streaming_metric_on_single_image(self):
        ...
        y_pred = combine_maps(pred_classes, pred_instances, max_instances_per_category)
    ...
    def test_streaming_metric_on_multiple_images(self):
        ...
        for test_image in test_images:
            ...
            pred_instances.append(
                test_utils.read_test_image(test_image.pred_inst_path, image_format="L")
            )
        ...
        for pred_class, pred_instance, gt_class, gt_instance in zip(
            ...
            y_pred = combine_maps(pred_class, pred_instance, max_instances_per_category)
            pq_obj.update_state(y_true, y_pred)
        result = pq_obj.result().numpy()
        ...
        self.assertAlmostEqual(result[4], 1.0 / 6, places=4)
        self.assertEqual(result[5], 0.0)
    ...
    def test_predicted_non_contiguous_ignore_label(self):
        ...
        y_pred = combine_maps(
            np.array(pred_class), np.array(pred_instance), max_instances_per_category
        )
        logging.info("y_pred=\n%s", y_pred)
        ...
        # pq
        self.assertAlmostEqual(result[0], 2.0 / 9, places=4)
        # sq
        self.assertAlmostEqual(result[1], 1.0 / 3, places=4)
        # rq
        self.assertAlmostEqual(result[2], 2.0 / 9, places=4)
        # tp
        self.assertAlmostEqual(result[3], 1.0 / 3, places=4)
        # fn
        self.assertAlmostEqual(result[4], 2.0 / 3, places=4)
        # fp
        self.assertAlmostEqual(result[5], 2.0 / 3, places=4)
