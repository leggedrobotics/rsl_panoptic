...
flags.DEFINE_string(
    "target_dataset",
    "motchallenge_step",
    "Dataset name for conversion. Supported datasets: " "`motchallenge_step`.",
)

flags.DEFINE_string(
    "input_config_path",
    None,
    "Path to a config file that defines the DeepLab model and " "the checkpoint path.",
)

flags.DEFINE_string(
    "output_checkpoint_path", None, "Output filename for the generated checkpoint file."
)
...
def _load_model(
    config_path: Text, source_dataset: Text
) -> Tuple[deeplab.DeepLab, config_pb2.ExperimentOptions]:
    ...
    model = deeplab.DeepLab(options, _DATASET_TO_INFO[source_dataset])
    return model, options


def _convert_bias(input_tensor: np.ndarray, label_list: Tuple[int, ...]) -> np.ndarray:
...
def _convert_kernels(
    input_tensor: np.ndarray, label_list: Tuple[int, ...]
) -> np.ndarray:
...
def _restore_checkpoint(
    restore_dict: Dict[Any, Any], options: config_pb2.ExperimentOptions
) -> tf.train.Checkpoint:
    ...
    if tf.io.gfile.isdir(options.model_options.initial_checkpoint):
        path = tf.train.latest_checkpoint(options.model_options.initial_checkpoint)
...
def main(_) -> None:
    if FLAGS.source_dataset not in _SUPPORTED_SOURCE_DATASETS:
        raise ValueError(
            "Source dataset is not supported. Use --help to get list "
            "of supported datasets."
        )
    if FLAGS.target_dataset not in _SUPPORTED_TARGET_DATASETS:
        raise ValueError(
            "Target dataset is not supported. Use --help to get list "
            "of supported datasets."
        )

    logging.info("Loading DeepLab model from config %s", FLAGS.input_config_path)
    source_model, options = _load_model(FLAGS.input_config_path, FLAGS.source_dataset)
    ...
    if (
        FLAGS.source_dataset == "cityscapes"
        and FLAGS.target_dataset == "motchallenge_step"
    ):
        # Kernels.
        semantic_weights[0] = _convert_kernels(
            semantic_weights[0], _CITYSCAPES_TO_MOTCHALLENGE_STEP
        )
        # Bias.
        semantic_weights[1] = _convert_bias(
            semantic_weights[1], _CITYSCAPES_TO_MOTCHALLENGE_STEP
        )
    ...
    target_model._decoder._semantic_head.final_conv.set_weights(
        semantic_weights
    )  # pylint: disable=protected-access

    logging.info("Save checkpoint to output path: %s", FLAGS.output_checkpoint_path)
    ckpt = tf.train.Checkpoint(**target_model.checkpoint_items)
    ckpt.save(FLAGS.output_checkpoint_path)


if __name__ == "__main__":
    flags.mark_flags_as_required(["input_config_path", "output_checkpoint_path"])
    app.run(main)
