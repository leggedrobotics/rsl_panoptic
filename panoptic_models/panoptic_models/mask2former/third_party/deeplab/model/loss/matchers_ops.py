...
def _greedy_assignment(adj_matrix):
    ...
    def _assign_row(accumulator, row_adj):
        ...
        candidates_indicator = tf.one_hot(
            max_candidate_idx, num_elems, on_value=True, off_value=False, dtype=tf.bool
        )
...
def _find_augmenting_path(assignment, adj_matrix):
    ...
    init_workers = tf.tile(
        worker_indices[tf.newaxis, :, tf.newaxis], [batch_size, 1, 1]
    )
    ...
    def _augment_step(state, curr_workers):
        ...
        state["jobs_from_worker"] = tf.maximum(
            state["jobs_from_worker"], curr_jobs_from_worker
        )
        state["workers"] = tf.maximum(state["workers"], curr_workers)
        state["workers_from_job"] = tf.maximum(
            state["workers_from_job"], curr_workers_from_job
        )
    ...
    with tf.name_scope("find_augmenting_path"):
        state, _ = tf.while_loop(
            _has_active_workers, _augment_step, (state, init_workers), back_prop=False
        )
...
def _improve_assignment(assignment, state):
    ...
    def _backtrack_one_step(flip_matrix, active, curr_job_idx):
        ...
        curr_worker_idx = (
            tf.gather(state["jobs_from_worker"], curr_job_idx, batch_dims=1) - 1
        )
        curr_worker_idx = tf.maximum(curr_worker_idx, 0)
        update_indices = tf.stack([batch_range, curr_worker_idx, curr_job_idx], axis=1)
        update_indices = tf.maximum(update_indices, 0)
        flip_matrix = tf.tensor_scatter_nd_add(
            flip_matrix, update_indices, tf.cast(active, tf.int32)
        )

        # Discover the (potential) job that the worker originated from.
        curr_job_idx = (
            tf.gather(state["workers_from_job"], curr_worker_idx, batch_dims=1) - 1
        )
...
def _update_weights_using_cover(workers_cover, jobs_cover, weights):
    ...
    uncovered_weights = tf.where(covered, tf.ones_like(weights) * max_value, weights)
    min_weight = tf.reduce_min(uncovered_weights, axis=[-2, -1], keepdims=True)

    add_weight = tf.where(
        double_covered, tf.ones_like(weights) * min_weight, tf.zeros_like(weights)
    )
    sub_weight = tf.where(
        covered, tf.zeros_like(weights), tf.ones_like(weights) * min_weight
    )
...
def get_shape_list(tensor, expected_rank=None):
    ...
    if expected_rank and actual_rank != expected_rank:
        raise ValueError(
            "The tensor has rank %d which is not equal to the "
...
def hungarian_matching(weights):
    ...
    adj_matrix = tf.equal(weights, 0.0)
    ...
    def _cover_incomplete(workers_cover, jobs_cover, *args):
        del args
        cover_sum = tf.reduce_sum(tf.cast(workers_cover, tf.int32)) + tf.reduce_sum(
            tf.cast(jobs_cover, tf.int32)
        )
