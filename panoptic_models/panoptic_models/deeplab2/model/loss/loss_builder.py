...
from panoptic_seg.deeplab2.model.loss import max_deeplab_loss


def _create_loss_and_weight(
    loss_options: config_pb2.LossOptions.SingleLossOptions,
    gt_key: Text,
    pred_key: Text,
    weight_key: Text,
    **kwargs: Any
    ...
    if loss_options.name == "softmax_cross_entropy":
        return (
            base_loss.TopKCrossEntropyLoss(
                gt_key,
                pred_key,
                weight_key,
        ...
        return (
            base_loss.SILogPlusRelativeSquaredLoss(gt_key, pred_key, **kwargs),
            loss_options.weight,
        )
...
class DeepLabFamilyLoss(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if loss_options.HasField(common.REGRESSION_LOSS):
            self._single_term_loss_func_and_weight_dict[
                common.REGRESSION_LOSS
            ] = _create_loss_and_weight(
                loss_options.regression_loss,
        ...
        if any(
            [
                loss_options.HasField("pq_style_loss"),
    ...
    def call(
        ...
        for (
            loss_name,
            func_and_weight,
        ) in self._single_term_loss_func_and_weight_dict.items():
        ...
        for multi_term_loss in self._multi_term_losses:
            ...
            if not set(loss_dict).isdisjoint(resulting_dict):
                raise AssertionError(
                    "The keys of the resulting_dict overlap with the "
