...
from panoptic_models.deeplab2.model.layers import blocks
from panoptic_models.deeplab2.model.layers import convolutions
...
MNV3Small_BLOCK_SPECS = {
    ...
    "block_specs": [
        ("conv_bn", 3, 2, 16, "hard_swish", None, None, True),
        ("inverted_bottleneck", 3, 2, 16, "relu", 0.25, 1, True),
        ("inverted_bottleneck", 3, 2, 24, "relu", None, 72.0 / 16, False),
        ("inverted_bottleneck", 3, 1, 24, "relu", None, 88.0 / 24, True),
        ("inverted_bottleneck", 5, 2, 40, "hard_swish", 0.25, 4.0, False),
        ("inverted_bottleneck", 5, 1, 40, "hard_swish", 0.25, 6.0, False),
        ("inverted_bottleneck", 5, 1, 40, "hard_swish", 0.25, 6.0, False),
        ("inverted_bottleneck", 5, 1, 48, "hard_swish", 0.25, 3.0, False),
        ("inverted_bottleneck", 5, 1, 48, "hard_swish", 0.25, 3.0, True),
        ("inverted_bottleneck", 5, 2, 96, "hard_swish", 0.25, 6.0, False),
        ("inverted_bottleneck", 5, 1, 96, "hard_swish", 0.25, 6.0, False),
        ("inverted_bottleneck", 5, 1, 96, "hard_swish", 0.25, 6.0, False),
        ("conv_bn", 1, 1, 576, "hard_swish", None, None, True),
    ],
}
...
MNV3Large_BLOCK_SPECS = {
    ...
    "block_specs": [
        ("conv_bn", 3, 2, 16, "hard_swish", None, None, False),
        ("inverted_bottleneck", 3, 1, 16, "relu", None, 1.0, True),
        ("inverted_bottleneck", 3, 2, 24, "relu", None, 4.0, False),
        ("inverted_bottleneck", 3, 1, 24, "relu", None, 3.0, True),
        ("inverted_bottleneck", 5, 2, 40, "relu", 0.25, 3.0, False),
        ("inverted_bottleneck", 5, 1, 40, "relu", 0.25, 3.0, False),
        ("inverted_bottleneck", 5, 1, 40, "relu", 0.25, 3.0, True),
        ("inverted_bottleneck", 3, 2, 80, "hard_swish", None, 6.0, False),
        ("inverted_bottleneck", 3, 1, 80, "hard_swish", None, 2.5, False),
        ("inverted_bottleneck", 3, 1, 80, "hard_swish", None, 2.3, False),
        ("inverted_bottleneck", 3, 1, 80, "hard_swish", None, 2.3, False),
        ("inverted_bottleneck", 3, 1, 112, "hard_swish", 0.25, 6.0, False),
        ("inverted_bottleneck", 3, 1, 112, "hard_swish", 0.25, 6.0, True),
        ("inverted_bottleneck", 5, 2, 160, "hard_swish", 0.25, 6.0, False),
        ("inverted_bottleneck", 5, 1, 160, "hard_swish", 0.25, 6.0, False),
        ("inverted_bottleneck", 5, 1, 160, "hard_swish", 0.25, 6.0, False),
        ("conv_bn", 1, 1, 960, "hard_swish", None, None, True),
    ],
}
...
def _block_spec_decoder(
    ...
    for ds in decoded_specs:
        ds["filters"] = utils.make_divisible(
            value=ds["filters"] * width_multiplier, divisor=divisible_by, min_value=8
        )
...
class MobileNet(tf.keras.Model):
    ...
    def __init__(
        ...
        if model_id not in SUPPORTED_SPECS_MAP:
            raise ValueError(
                "The MobileNet version {} " "is not supported".format(model_id)
            )
    ...
    def _mobilenet_base(self):
        ...
        for i, block_def in enumerate(self._decoded_specs):
            ...
            if (
                self._output_stride is not None
                ...
                rate = (
                    ...
                    if block_def["strides"] is not None
                    else rate
                )
            else:
