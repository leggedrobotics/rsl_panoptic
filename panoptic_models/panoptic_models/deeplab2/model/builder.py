...
from panoptic_models.deeplab2.model.decoder import deeplabv3
from panoptic_models.deeplab2.model.decoder import deeplabv3plus
from panoptic_models.deeplab2.model.decoder import max_deeplab
from panoptic_models.deeplab2.model.decoder import motion_deeplab_decoder
from panoptic_models.deeplab2.model.decoder import panoptic_deeplab
from panoptic_models.deeplab2.model.decoder import vip_deeplab_decoder
from panoptic_models.deeplab2.model.encoder import axial_resnet_instances
from panoptic_models.deeplab2.model.encoder import mobilenet
...
def create_encoder(
    ...
    """
    if (
        "resnet" in backbone_options.name
        or "swidernet" in backbone_options.name
        or "axial_deeplab" in backbone_options.name
        or "max_deeplab" in backbone_options.name
    ):
    ...
    raise ValueError(
        "The specified encoder %s is not a valid encoder." % backbone_options.name
    )
...
def create_decoder(
    model_options: config_pb2.ModelOptions,
    bn_layer: tf.keras.layers.Layer,
    ignore_label: int,
) -> tf.keras.Model:
    ...
    if meta_architecture == "deeplab_v3":
        ...
        return panoptic_deeplab.PanopticDeepLab(
            model_options.decoder, model_options.panoptic_deeplab, bn_layer=bn_layer
        )
    elif meta_architecture == "motion_deeplab":
        return motion_deeplab_decoder.MotionDeepLabDecoder(
            model_options.decoder, model_options.motion_deeplab, bn_layer=bn_layer
        )
    elif meta_architecture == "vip_deeplab":
        return vip_deeplab_decoder.ViPDeepLabDecoder(
            model_options.decoder, model_options.vip_deeplab, bn_layer=bn_layer
        )
    ...
    raise ValueError(
        "The specified meta architecture %s is not implemented." % meta_architecture
    )
